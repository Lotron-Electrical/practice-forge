...
export function ThemeProvider({ children }: Props) {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById("pf-cvd-svg")) {
      ...
      for (const [mode, m] of Object.entries(matrices)) {
        ...
        const feMatrix = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "feColorMatrix",
        );
  ...
  const setColourVisionMode = (v: string) => {
    setColourVisionModeState(v);
    persist("colourVisionMode", v);
  };
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        if (data.colourVisionMode)
          setColourVisionModeState(data.colourVisionMode);
        if (typeof data.reducedMotion === "boolean")
          setReducedMotionState(data.reducedMotion);
