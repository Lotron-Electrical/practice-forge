...
export function ResourceFinderPanel({ linkedType, linkedId, title, composer }: ResourceFinderPanelProps) {
  const [resources, setResources] = useState<Resource[]>([]);
  const [showModal, setShowModal] = useState(false);

  const load = useCallback(() => {
    api.getResources({ linked_type: linkedType, linked_id: linkedId })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-semibold text-[var(--pf-text-secondary)]">Resources</h3>
        <button
      ...
      {resources.length === 0 && (
        <p className="text-xs text-[var(--pf-text-secondary)]">No resources linked.</p>
      )}
