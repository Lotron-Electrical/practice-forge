...
export function ResourceSearchModal({
  ...
  const doAutoDiscover = async () => {
    ...
    setResults([]);
    try {
      const data = (await api.autoDiscover({ title: query.trim() })) as {
            ...
            <>
              ...
              {loading && (
                <div className="flex justify-center py-6">
                  <Loader2
                    size={20}
                    className="animate-spin text-[var(--pf-text-secondary)]"
                  />
                </div>
              )}
              ...
              {results.map((r, i) => (
                <div
                  key={`${r.url}-${i}`}
                  className="flex items-start gap-3 p-2 border border-[var(--pf-border-color)] rounded-pf-sm"
                >
                  {r.thumbnail_url && (
                    <img
                      src={r.thumbnail_url}
                  ...
                  <div className="flex-1 min-w-0">
                    <a
                      ...
                      className="text-xs font-medium hover:underline text-[var(--pf-text-primary)] flex items-center gap-1"
                    >
                      {r.title} <ExternalLink size={10} />
                    </a>
                    {r.description && (
                      <p className="text-xs text-[var(--pf-text-secondary)] mt-0.5 line-clamp-2">
                        {r.description}
                      </p>
                    )}
                    <span className="text-[10px] text-[var(--pf-text-secondary)] uppercase">
                      {r.source}
                    </span>
                  </div>
                  <Button
