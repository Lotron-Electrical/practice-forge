...
import type { AudioAnalysisData, ProblemSpot, BarResult } from '../../core/types';
...
interface Props {
  ...
  summary: {
    ...
    note_events: { time: number; frequency: number; note: string; duration: number; velocity: number }[];
  };
...
export function PostPlayReport({
  duration, audioBlob, summary, analysis, barResults, onSave, onDiscard, isSaving,
}: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card>
          <CardContent className="text-center py-3">
            <div className="text-2xl font-bold" style={{ color: pitchColor(summary.pitch_accuracy_pct) }}>
              {summary.pitch_accuracy_pct}%
            </div>
            <div className="text-xs text-[var(--pf-text-secondary)]">Pitch Accuracy</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="text-center py-3">
            <div className="text-2xl font-bold" style={{ color: 'var(--pf-accent-teal)' }}>
              {summary.dynamic_range_db > 0 ? summary.dynamic_range_db.toFixed(1) : '—'}
            </div>
            <div className="text-xs text-[var(--pf-text-secondary)]">Dynamic Range (dB)</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="text-center py-3">
            <div className="text-2xl font-bold" style={{ color: summary.pitch_stability < 15 ? 'var(--pf-status-ready)' : 'var(--pf-accent-gold)' }}>
              {summary.pitch_stability}
            </div>
            <div className="text-xs text-[var(--pf-text-secondary)]">Pitch Stability</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="text-center py-3">
            ...
            <div className="text-xs text-[var(--pf-text-secondary)] mt-1">Overall</div>
          </CardContent>
        </Card>
      </div>
      ...
      {analysis.pitch_trace.length > 0 && (
        <Card>
          <CardHeader><h3 className="text-sm font-semibold">Pitch Trace</h3></CardHeader>
          <CardContent>
            <div className="flex items-end gap-px h-20 overflow-hidden">
              {analysis.pitch_trace
                .filter((_, i) => i % Math.max(1, Math.floor(analysis.pitch_trace.length / 200)) === 0)
                .map((sample, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        width: 2,
                        height,
                        backgroundColor: cents <= 15 ? 'var(--pf-status-ready)' : cents <= 30 ? 'var(--pf-accent-gold)' : 'var(--pf-status-needs-work)',
                        opacity: 0.8,
                      }}
                    />
                  );
      ...
      {analysis.dynamics_envelope.length > 0 && (
        <Card>
          <CardHeader><h3 className="text-sm font-semibold">Dynamics</h3></CardHeader>
          <CardContent>
            <div className="flex items-end gap-px h-16 overflow-hidden">
              {analysis.dynamics_envelope
                .filter((_, i) => i % Math.max(1, Math.floor(analysis.dynamics_envelope.length / 200)) === 0)
                .map((sample, i) => {
      ...
      {summary.problem_spots.length > 0 && (
        <Card>
          <CardHeader><h3 className="text-sm font-semibold">Problem Spots ({summary.problem_spots.length})</h3></CardHeader>
          <CardContent className="space-y-2">
            {summary.problem_spots.slice(0, 10).map((spot, i) => (
              <div key={i} className="flex items-center gap-3 text-sm">
                <AlertTriangle size={14} style={{
                  color: spot.severity === 'major' ? 'var(--pf-status-needs-work)' :
                    spot.severity === 'moderate' ? 'var(--pf-accent-gold)' : 'var(--pf-text-secondary)',
                }} />
                <div className="flex-1">
                  <span className="text-[var(--pf-text-primary)]">{spot.description}</span>
                  <span className="text-xs text-[var(--pf-text-secondary)] ml-2">
                    {formatTime(spot.time_start)} — {formatTime(spot.time_end)}
                  </span>
                </div>
                <Badge color={
                  spot.severity === 'major' ? 'var(--pf-status-needs-work)' :
                    spot.severity === 'moderate' ? 'var(--pf-accent-gold)' : 'var(--pf-text-secondary)'
                }>
                  {spot.severity}
                </Badge>
      ...
      {barResults && barResults.length > 0 && (
        <Card>
          <CardHeader><h3 className="text-sm font-semibold">Bar-by-Bar Results</h3></CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-1">
              {barResults.map(bar => (
                <div
                  ...
                  title={`Bar ${bar.bar_number}: ${bar.pitch_accuracy}% accuracy`}
                  style={{
                    backgroundColor: bar.status === 'accurate' ? '#22c55e' :
                      bar.status === 'minor_issues' ? '#eab308' : '#ef4444',
                  }}
                >
                  {bar.bar_number}
                </div>
      ...
      <Card>
        <CardContent className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-[var(--pf-text-secondary)]">{formatTime(duration)}</span>
