...
export function RecordingsList({
  ...
  const load = useCallback(() => {
    ...
    api
      ...
      .then((data) => setRecordings(data as AudioRecording[]))
      .catch(() => {});
          ...
          <Card key={rec.id} className="overflow-hidden">
            <CardContent className="py-2">
              ...
              >
                <Mic
                  size={14}
                  className="text-[var(--pf-text-secondary)] flex-shrink-0"
                />
              ...
              {isExpanded && (
                <div className="mt-3 pt-3 border-t border-[var(--pf-border-color)] space-y-2">
                  {rec.file_id && (
                    <audio
                      controls
                      src={api.getRecordingAudioUrl(rec.file_id)}
                  ...
                  {rec.analysis_rating && (
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <Badge
                        color={
                          rec.analysis_rating === "excellent"
                            ? "var(--pf-status-ready)"
                            : rec.analysis_rating === "solid"
                              ? "var(--pf-status-in-progress)"
                              : rec.analysis_rating === "acceptable"
                                ? "var(--pf-accent-gold)"
