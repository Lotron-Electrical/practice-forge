...
const MODES: Array<{ key: HighlightMode; label: string; icon: typeof Waves; color: string }> = [
...
export function AnalysisToolbar({ mode, onChange, counts }: AnalysisToolbarProps) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-xs font-medium text-[var(--pf-text-secondary)] mr-1">Highlight:</span>
      {MODES.map(m => {
        const Icon = m.icon;
        ...
        return (
          <button
            ...
            style={active ? { color: m.color } : { color: 'var(--pf-text-secondary)' }}
          >
            <Icon size={14} />
            {m.label}
      ...
      {mode !== 'none' && (
        <button onClick={() => onChange('none')} className="p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-text-primary)]">
          <X size={14} />
        </button>
      )}
    </div>
