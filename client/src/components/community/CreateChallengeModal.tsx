...
export function CreateChallengeModal({ open, onClose, onCreated }: Props) {
  ...
  const [searchResults, setSearchResults] = useState<
    { id: string; display_name: string; instrument?: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(async () => {
      setSearching(true);
      try {
        const results = (await api.searchUsers(searchQuery)) as {
          id: string;
          display_name: string;
          instrument?: string;
        }[];
        setSearchResults(
          results.filter((r) => !opponents.some((o) => o.id === r.id)),
        );
      } catch {
        setSearchResults([]);
      }
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            {challengeType === "excerpt_duel" && (
              <div>
                <label className="text-sm font-medium text-[var(--pf-text-secondary)] mb-1 block">
                  Excerpt
                </label>
                <select
            ...
            <div>
              ...
              <div className="relative">
                ...
                {searching && (
                  <Loader
                    size={14}
                    className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-[var(--pf-text-secondary)]"
                  />
                )}
              </div>
                ...
                <div className="mt-1 border border-[var(--pf-border-color)] rounded-pf-sm overflow-hidden max-h-32 overflow-y-auto">
                  {searchResults.map((u) => (
                    ...
                    >
                      <Plus
                        size={14}
                        className="text-[var(--pf-accent-teal)]"
                      />
