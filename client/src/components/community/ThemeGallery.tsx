        ...
        prev.map(t =>
          t.id === theme.id
            ? {
                ...
                favorites_count: t.is_favorited ? t.favorites_count - 1 : t.favorites_count + 1,
              }
            : t
        )
      );
      ...
      {themes.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            <Palette size={32} className="mx-auto mb-3 text-[var(--pf-text-secondary)]" />
            <p className="text-sm text-[var(--pf-text-secondary)]">No community themes yet. Be the first to share one.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={theme.id}>
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  {theme.creator_name && (
                    <p className="text-xs text-[var(--pf-text-secondary)]">by {theme.creator_name}</p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="ml-auto flex gap-1">
                    <Button size="sm" variant="secondary" onClick={() => handleApply(theme)}>
                      Apply
                    </Button>
                    <button
                    ...
                    >
                      <Heart size={16} fill={theme.is_favorited ? 'currentColor' : 'none'} />
                    </button>
                  </div>
                </div>
              </CardContent>
