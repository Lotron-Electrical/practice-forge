        ...
        <Card>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={onClose} className="text-[var(--pf-text-secondary)]">
                <X size={18} />
              </button>
            </div>
            ...
            {/* Base theme */}
            <div>
              <label className="text-sm font-medium text-[var(--pf-text-secondary)] mb-2 block">Base theme</label>
            ...
            {/* Colour pickers */}
            <div>
              <label className="text-sm font-medium text-[var(--pf-text-secondary)] mb-2 block">Colours</label>
                ...
                {TOKEN_FIELDS.map(({ key, label }) => (
                  <div key={key} className="flex items-center gap-2">
                    <input
                      type="color"
                      value={colours[key]}
                      onChange={e => setColours(prev => ({ ...prev, [key]: e.target.value }))}
            ...
            {/* Live preview */}
            <div>
              <label className="text-sm font-medium text-[var(--pf-text-secondary)] mb-2 block">Preview</label>
              <div
                className="rounded-pf border border-[var(--pf-border-color)] p-4 space-y-2"
                style={{ backgroundColor: colours['--pf-bg-primary'] }}
              >
                <p style={{ color: colours['--pf-text-primary'], fontWeight: 600 }}>
                  {name || 'Theme Name'}
                </p>
                <p style={{ color: colours['--pf-text-primary'], fontSize: '0.875rem', opacity: 0.7 }}>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button size="sm" onClick={handlePublish} disabled={!name.trim() || publishing}>
