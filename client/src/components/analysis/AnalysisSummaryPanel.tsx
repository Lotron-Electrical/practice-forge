      ...
      <CardContent className="space-y-3 text-sm">
        {analysis.key_signature && (
          <div className="flex items-center gap-2">
            ...
            <span className="ml-auto font-medium">{analysis.key_signature}</span>
          </div>
        )}
        {analysis.time_signature && (
          <div className="flex items-center gap-2">
            ...
            <span className="ml-auto font-medium">{analysis.time_signature}</span>
          </div>
        )}
        {analysis.tempo_marking && (
          <div className="flex items-center gap-2">
            ...
            <span className="ml-auto font-medium">{analysis.tempo_marking}</span>
          </div>
        )}
        {analysis.difficulty_estimate && (
          <div className="flex items-center gap-2">
            <span className="text-[var(--pf-text-secondary)]">Difficulty</span>
            <span className="ml-auto"><DifficultyDots value={analysis.difficulty_estimate} /></span>
          </div>
        )}
        {(analysis.register_low || analysis.register_high) && (
          <div className="flex items-center gap-2">
            <ArrowUpDown size={14} className="text-[var(--pf-text-secondary)]" />
            <span className="text-[var(--pf-text-secondary)]">Register</span>
            <span className="ml-auto font-medium">{analysis.register_low} — {analysis.register_high}</span>
          </div>
        )}
        {analysis.total_measures && (
          <div className="flex items-center gap-2">
            <span className="text-[var(--pf-text-secondary)]">Measures</span>
            <span className="ml-auto font-medium">{analysis.total_measures}</span>
          </div>
        )}
        {analysis.analysis_data?.rhythm && (
          <div className="flex items-center gap-2">
            <span className="text-[var(--pf-text-secondary)]">Rhythm complexity</span>
            <span className="ml-auto font-medium">{analysis.analysis_data.rhythm.complexity_score}/10</span>
          </div>
        )}
      </CardContent>
    </Card>
