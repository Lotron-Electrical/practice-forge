...
export function GeneratedExerciseCard({
  ...
  const autoCategory = exercise.category_hint
    ? categories.find((c) =>
        c.name.toLowerCase().includes(exercise.category_hint!.toLowerCase()),
      )
    : null;
  ...
  return (
    <Card
      borderColor={isAi ? "var(--pf-accent-gold)" : "var(--pf-accent-teal)"}
    >
      <CardContent className="space-y-3">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
              {isAi ? (
                <Sparkles size={14} className="text-[var(--pf-accent-gold)]" />
              ) : (
                <Settings size={14} className="text-[var(--pf-accent-teal)]" />
              )}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Badge
            color={isAi ? "var(--pf-accent-gold)" : "var(--pf-accent-teal)"}
          >
            {isAi ? "AI" : "Rule"}
          </Badge>
          {exercise.key && (
            <Badge color="var(--pf-accent-lavender)">{exercise.key}</Badge>
          )}
          {exercise.difficulty && (
