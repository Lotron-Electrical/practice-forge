...
export function GenerateExerciseModal({
  ...
  const handleRuleGenerate = async () => {
    ...
    setResult(null);
    try {
      const params: RuleGenerationParams = {
        key: ruleKey,
        tempo: ruleTempo,
        octaves: ruleOctaves,
      };
      if (ruleType === "scale") {
        params.scaleType = ruleScaleType;
        params.pattern = rulePattern;
      }
      if (ruleType === "arpeggio") {
        params.chordType = ruleChordType;
      }
      if (ruleType === "interval") {
        params.interval = ruleInterval;
      }
      if (ruleType === "articulation") {
        params.articulation = ruleArticulation;
      }

      const data = (await api.generateRule(
        ruleType,
        params,
  ...
  const handleDemandGenerate = async () => {
    ...
    setResult(null);
    try {
      const data = (await api.generateFromDemand(
        demandId,
      )) as GeneratedExercise;
      setResult(data);
    } catch (err) {
  ...
  const confirmAiGenerate = async () => {
    ...
    try {
      const data = (await api.generateAi(
        pendingAiRequest.type,
        pendingAiRequest.prompt,
        pendingAiRequest.context,
        true,
      )) as GeneratedExercise;
      setResult(data);
    } catch (err) {
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Select
                    ...
                    value={ruleType}
                    onChange={(e) =>
                      setRuleType(e.target.value as GenerationType)
                    }
                  >
                ...
                {ruleType === "scale" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <Select
                      label="Scale type"
                      value={ruleScaleType}
                      onChange={(e) => setRuleScaleType(e.target.value)}
                    >
                      {SCALE_TYPES.map((s) => (
                ...
                {ruleType === "arpeggio" && (
                  <Select
                    label="Chord type"
                    value={ruleChordType}
                    onChange={(e) => setRuleChordType(e.target.value)}
                  >
                    {CHORD_TYPES.map((c) => (
                ...
                {ruleType === "articulation" && (
                  <Select
                    label="Articulation"
                    value={ruleArticulation}
                    onChange={(e) => setRuleArticulation(e.target.value)}
                  >
                    {ARTICULATIONS.map((a) => (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Input
                    ...
                    value={ruleOctaves}
                    onChange={(e) =>
                      setRuleOctaves(parseInt(e.target.value) || 2)
                    }
                  />
                  <Input
                    label="Tempo (BPM)"
                    ...
                    value={ruleTempo}
                    onChange={(e) =>
                      setRuleTempo(parseInt(e.target.value) || 80)
                    }
                  />
                </div>
            ...
            {tab === "ai" && (
              <div className="space-y-3">
                ...
                >
                  ...
                  <option value="excerpt_prep">
                    Excerpt Preparation Routine
                  </option>
