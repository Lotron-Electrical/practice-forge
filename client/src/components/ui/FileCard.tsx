...
export function FileCard({ file, onEdit, onDelete }: FileCardProps) {
  ...
  const statusColor =
    STATUS_COLORS[file.processing_status] || "var(--pf-text-secondary)";
    ...
    <div className="p-4 rounded-pf border border-[var(--pf-border-color)] bg-[var(--pf-bg-card)] hover:shadow-pf-lg transition-shadow group">
      <div className="flex items-start gap-3">
        ...
        <div className="flex sm:hidden sm:group-hover:flex gap-1">
          {(file.file_type === "sheet_music_digital" ||
            file.file_type === "sheet_music_scanned") && (
            <button
              onClick={() => navigate(`/scores/${file.id}`)}
              className="p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-accent-teal)]"
              title="Analyse"
            >
              <Sparkles size={14} />
            </button>
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        ...
        {file.linked_type && (
          <span
            className="text-xs px-1.5 py-0.5 rounded"
            style={{ backgroundColor: "var(--pf-accent-teal)", color: "white" }}
          >
            {file.linked_type}
          </span>
        )}
