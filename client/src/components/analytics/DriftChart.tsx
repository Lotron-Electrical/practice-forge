...
export function DriftChart({ data }: { data: DriftData }) {
  if (!data.hasData) {
    return <p className="text-sm text-[var(--pf-text-secondary)] text-center py-8">Complete sessions in the last 2 weeks to see allocation drift.</p>;
  }
        ...
        return (
          <div key={cat}>
            <div className="flex items-center justify-between text-xs mb-1">
              ...
              <span className={overDrift ? 'text-[var(--pf-status-needs-work)] font-medium' : 'text-[var(--pf-text-secondary)]'}>
            ...
            <div className="relative h-4 rounded-full overflow-hidden bg-[var(--pf-bg-hover)]">
              ...
              <div
                ...
                style={{
                  ...
                  backgroundColor: overDrift ? 'var(--pf-status-needs-work)' : 'var(--pf-accent-gold)',
                }}
              />
            </div>
          </div>
        ...
        <div className="mt-4 pt-3 border-t border-[var(--pf-border-color)] space-y-1.5">
          {data.alerts.map((alert, i) => (
            <div key={i} className="flex items-start gap-2 text-xs">
              <div className="w-1 h-4 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: 'var(--pf-status-needs-work)' }} />
              <span>{alert}</span>
            </div>
          ))}
        </div>
