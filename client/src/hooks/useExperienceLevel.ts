  ...
  useEffect(() => {
    api
      ...
      .then((data: Record<string, unknown>) => {
        ...
        if (
          serverLevel === "beginner" ||
          serverLevel === "intermediate" ||
          serverLevel === "advanced"
        ) {
...
export function isPathAllowed(path: string, level: ExperienceLevel): boolean {
  ...
  switch (level) {
    case "beginner":
      return BEGINNER_PATHS.has(normalized);
    case "intermediate":
      return INTERMEDIATE_PATHS.has(normalized);
    case "advanced":
      return true;
  }
}
...
export function isNavItemAllowed(
  ...
  switch (level) {
    case "beginner":
      return BEGINNER_LABELS.has(label);
    case "intermediate":
      return INTERMEDIATE_LABELS.has(label);
    case "advanced":
      return true;
  }
}
