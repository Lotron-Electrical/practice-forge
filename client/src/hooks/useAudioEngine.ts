...
import type { PitchSample, DynamicsSample, ProblemSpot, DetectedNote, AudioAnalysisData } from '../core/types';
...
export function useAudioEngine() {
  ...
  const [currentDynamics, setCurrentDynamics] = useState<DynamicsSample | null>(null);
  ...
  const meydaFeaturesRef = useRef<{ rms: number; spectralCentroid: number; spectralFlatness: number }>({
    rms: 0, spectralCentroid: 0, spectralFlatness: 0,
  });
  ...
  const cleanup = useCallback(() => {
    ...
    if (meydaRef.current) { try { meydaRef.current.stop(); } catch {} }
    ...
    try {
      ...
      timerRef.current = setInterval(() => {
        if (!isPausedRef.current) {
          const elapsed = (performance.now() - startTimeRef.current - pausedDurationRef.current) / 1000;
          setDuration(elapsed);
        }
      }, 200);
      ...
      const analysisLoop = () => {
        ...
        if (now - lastAnalysisRef.current >= 1000 / ANALYSIS_FPS && !isPausedRef.current) {
          ...
          if (clarity >= CLARITY_THRESHOLD && pitch >= MIN_FREQ && pitch <= MAX_FREQ) {
            ...
            const sample: PitchSample = {
              time: elapsed, frequency: pitch, note, cents_deviation: cents, clarity,
            };
            setCurrentPitch(sample);
            pitchTraceRef.current.push(sample);
            // Keep UI trace to last 10 seconds
            const recent = pitchTraceRef.current.filter(s => s.time > elapsed - 10);
            setPitchTrace([...recent]);
          } else {
            setCurrentPitch(null);
          }

          const { rms, spectralCentroid, spectralFlatness } = meydaFeaturesRef.current;
          const dynSample: DynamicsSample = {
            time: elapsed, rms, spectral_centroid: spectralCentroid, spectral_flatness: spectralFlatness,
          };
          setCurrentDynamics(dynSample);
          dynamicsRef.current.push(dynSample);
          const recentDyn = dynamicsRef.current.filter(s => s.time > elapsed - 10);
  ...
  const stopRecording = useCallback((): Promise<RecordingResult> => {
    return new Promise((resolve) => {
      ...
      const finalDuration = (performance.now() - startTimeRef.current - pausedDurationRef.current) / 1000;
      ...
      if (recorder && recorder.state !== 'inactive') {
        recorder.onstop = () => {
          ...
          const inTune = allPitch.filter(s => Math.abs(s.cents_deviation) <= 20).length;
          const pitchAccuracy = allPitch.length > 0 ? (inTune / allPitch.length) * 100 : 0;

          // Pitch stability: stddev of cents
          const centsMean = allPitch.reduce((s, p) => s + p.cents_deviation, 0) / (allPitch.length || 1);
          const centsVariance = allPitch.reduce((s, p) => s + (p.cents_deviation - centsMean) ** 2, 0) / (allPitch.length || 1);
          ...
          const minRms = Math.min(...rmsValues.filter(v => v > 0.001), maxRms);
          const dynamicRangeDb = 20 * Math.log10(maxRms / Math.max(minRms, 0.0001));

          const avgSpectralCentroid = allDynamics.reduce((s, d) => s + d.spectral_centroid, 0) / (allDynamics.length || 1);
          const avgSpectralFlatness = allDynamics.reduce((s, d) => s + d.spectral_flatness, 0) / (allDynamics.length || 1);
          ...
          for (const sample of allPitch) {
            if (Math.abs(sample.cents_deviation) > 30) {
              ...
              if (pitchProbStart !== null && sample.time - pitchProbStart > 0.3) {
                problemSpots.push({
                  time_start: pitchProbStart, time_end: sample.time,
                  type: 'pitch', description: 'Pitch deviation >30 cents',
                  severity: sample.time - pitchProbStart > 1 ? 'major' : 'moderate',
                });
              }
              pitchProbStart = null;
            }
          ...
          for (let i = 1; i < allDynamics.length; i++) {
            if (allDynamics[i].rms < avgRms * 0.1 && allDynamics[i].rms > 0) {
              ...
              let end = start;
              while (i < allDynamics.length && allDynamics[i].rms < avgRms * 0.1) {
              ...
              if (end - start > 0.5) {
                problemSpots.push({
                  ...
                  type: 'dynamics', description: 'Significant drop in volume',
                  severity: 'minor',
                });
              }
            }
          ...
          let currentNote: { freq: number; note: string; start: number; rmsSum: number; count: number } | null = null;
          for (const sample of allPitch) {
            if (!currentNote || Math.abs(12 * Math.log2(sample.frequency / currentNote.freq)) > 0.5) {
              if (currentNote) {
                noteEvents.push({
                  time: currentNote.start, frequency: currentNote.freq, note: currentNote.note,
                  duration: sample.time - currentNote.start,
                  velocity: currentNote.rmsSum / currentNote.count,
                });
              }
              currentNote = { freq: sample.frequency, note: sample.note, start: sample.time, rmsSum: 0, count: 0 };
            }
            const dynAtTime = allDynamics.find(d => Math.abs(d.time - sample.time) < 0.15);
          ...
          // Overall rating
          let rating: 'needs_work' | 'acceptable' | 'solid' | 'excellent' = 'needs_work';
        ...
        cleanup();
        resolve({
          audioBlob: new Blob(), duration: 0,
          analysis: { pitch_trace: [], dynamics_envelope: [], problem_spots: [] },
          summary: {
            pitch_accuracy_pct: 0, dynamic_range_db: 0, avg_rms: 0,
            avg_spectral_centroid: 0, avg_spectral_flatness: 0,
            pitch_stability: 0, overall_rating: 'needs_work',
            problem_spots: [], note_events: [],
          },
        });
      }
    });
  ...
  return {
    isRecording, isPaused, duration, currentPitch, currentDynamics,
    pitchTrace, dynamicsEnvelope,
