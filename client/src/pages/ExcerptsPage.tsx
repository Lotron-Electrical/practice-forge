...
export function ExcerptsPage() {
  ...
  const [form, setForm] = useState({
    ...
    location_in_score: "",
    difficulty: "",
    status: "needs_work" as ExcerptStatus,
    historical_context: "",
    performance_notes: "",
  });
  ...
  const saveNote = async (id: string) => {
    ...
    setExcerpts((prev) =>
      prev.map((ex) =>
        ex.id === id ? { ...ex, performance_notes: editingNoteValue } : ex,
      ),
    );
  };
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (ex) =>
          ...
          ex.composer.toLowerCase().includes(q),
      );
    }
  ...
  const confirmPrepGenerate = async () => {
    ...
    setPrepLoading(true);
    try {
      const data = (await api.generateExcerptPrep(
        prepExcerptId,
        true,
  ...
  const openEdit = (ex: Excerpt) => {
    ...
    setForm({
      ...
      full_work_title: ex.full_work_title,
      location_in_score: ex.location_in_score,
      difficulty: ex.difficulty?.toString() || "",
      status: ex.status,
      historical_context: ex.historical_context,
      performance_notes: ex.performance_notes,
    });
    setShowForm(true);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">
          {level === "beginner" ? "Practice Rotation" : "Orchestral Excerpts"}
        </h1>
        <Button size="sm" onClick={openCreate}>
          <Plus size={16} />{" "}
          {level === "beginner" ? "Add Passage" : "Add Excerpt"}
        </Button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative flex-1 min-w-[200px]">
          <Search
            size={16}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--pf-text-secondary)]"
          />
          <input
            type="text"
            value={searchQuery}
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold">
                {editingId ? "Edit Excerpt" : "New Excerpt"}
              </h2>
              <button
                onClick={() => {
                  setShowForm(false);
              ...
              >
                <X size={18} className="text-[var(--pf-text-secondary)]" />
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input
                ...
                placeholder="e.g. Brahms Symphony No. 4 — Mvt. 4 flute solo"
              />
              <Input
                label="Composer"
                value={form.composer}
              ...
              />
              <Input
                label="Full work title"
                value={form.full_work_title}
                onChange={(e) =>
                  setForm((f) => ({ ...f, full_work_title: e.target.value }))
                }
              />
              <Input
                label="Location in score"
                value={form.location_in_score}
                onChange={(e) =>
                  setForm((f) => ({ ...f, location_in_score: e.target.value }))
                }
                placeholder="e.g. Rehearsal D, bars 93-108"
              />
              <Input
              ...
              <Select
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    status: e.target.value as ExcerptStatus,
                  }))
                }
              >
                <option value="needs_work">Needs work</option>
            ...
            <Textarea
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, performance_notes: e.target.value }))
              }
            />
            <Textarea
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, historical_context: e.target.value }))
              }
            />
      ...
      {filteredExcerpts.length === 0 && !showForm ? (
        <Card>
          <CardContent className="text-center py-12">
            <ListMusic
              size={48}
              className="mx-auto mb-4 text-[var(--pf-text-secondary)]"
            />
        ...
        <div className="space-y-3">
          {filteredExcerpts.map((ex) => {
            ...
            return (
              <Card
                key={ex.id}
                borderColor={`var(${sConf.colorVar})`}
                className="group"
              >
                <CardContent className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-[var(--pf-text-secondary)] flex-wrap">
                      ...
                      {ex.location_in_score && (
                        <span>{ex.location_in_score}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      {editingNoteId === ex.id ? (
                        <div className="flex items-center gap-1 flex-1">
                          ...
                          />
                          <button
                            onClick={() => saveNote(ex.id)}
                            className="text-[var(--pf-text-secondary)] hover:text-[var(--pf-accent-primary)]"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex sm:hidden sm:group-hover:flex gap-1">
                      <button
                        onClick={() => handleGeneratePrep(ex.id)}
                        className="p-2.5 sm:p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-accent-gold)]"
                        title="Generate prep routine"
                        aria-label="Generate prep routine"
                        disabled={prepLoading && prepExcerptId === ex.id}
                      >
                        {prepLoading && prepExcerptId === ex.id ? (
                      ...
                      <button
                        onClick={() =>
                          openLogForm(logExcerptId === ex.id ? "" : ex.id)
                        }
                        className="p-2.5 sm:p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-accent-gold)]"
                        title="Log practice"
                      ...
                      <button
                        onClick={() =>
                          setExpandedResourceId(
                            expandedResourceId === ex.id ? null : ex.id,
                          )
                        }
                        className="p-2.5 sm:p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-accent-teal)]"
                        title="Resources"
                      ...
                      <button
                        onClick={() => openEdit(ex)}
                        className="p-2.5 sm:p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-text-primary)]"
                        aria-label="Edit"
                      >
                        <Pencil size={14} />
                      </button>
                        ...
                        {logRatings.map(
                          ({ value, icon: Icon, label, color }) => (
                            <button
                              key={value}
                              onClick={() =>
                                setLogRating(logRating === value ? null : value)
                              }
                ...
                {loggedId === ex.id && !logExcerptId && (
                  <div className="px-4 pb-3 pt-0 border-t border-[var(--pf-border-color)]">
                    <div
                      className="pt-2 flex items-center gap-1.5 text-xs font-medium"
                      style={{ color: "var(--pf-status-ready)" }}
                    >
      ...
      {prepResult && prepExcerptId && (
        <div
          ...
          onClick={() => {
            setPrepResult(null);
            setPrepExcerptId(null);
          }}
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-label="Preparation Routine"
