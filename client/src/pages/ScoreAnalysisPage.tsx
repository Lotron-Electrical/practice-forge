...
export function ScoreAnalysisPage() {
  ...
  const { omrResult, analysisResult, isProcessing, startPolling, refresh } =
    useAnalysisPolling(fileId);
  ...
  const loadDemands = useCallback(async () => {
    ...
    try {
      const d = (await api.getAnalysisDemands(
        analysisResult.id,
      )) as AnalysisDemand[];
      setDemands(d);
    } catch {
  ...
  const handleClaudeEnhance = async () => {
    if (!analysisResult?.id) return;
    try {
      const result = (await api.triggerClaudeEnhance(analysisResult.id)) as {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-[var(--pf-text-secondary)] mb-4">
        <Link to="/media" className="hover:text-[var(--pf-text-primary)]">
          Media Library
        </Link>
            ...
            <Card>
              <CardContent className="text-center py-8">
                <p className="text-[var(--pf-text-secondary)] mb-4">
                  This is a scanned score. Run OMR to convert it to MusicXML.
                </p>
                <Button onClick={handleTriggerOmr}>
                  Run OMR (Optical Music Recognition)
                </Button>
              </CardContent>
            </Card>
          )}
              ...
              {hasAnalysis && (
                <AnalysisToolbar
                  ...
                  counts={{
                    ...
                    arpeggios:
                      analysisResult.analysis_data?.arpeggios?.length || 0,
                    dynamics:
                      analysisResult.analysis_data?.dynamics?.length || 0,
                  }}
                />
              )}
              <Card>
                <CardContent className="p-0 overflow-hidden">
                  <ScoreViewer
                    ...
                    analysisData={
                      hasAnalysis ? analysisResult.analysis_data : undefined
                    }
            ...
            <div className="flex justify-center">
              <Button
                ...
                onClick={() => {
                  ...
                  navigate(
                    `/record?fileId=${fileId}&startBar=1&endBar=${totalMeasures}`,
                  );
                }}
              >
          ...
          {analysisResult?.claude_analysis && (
            <Card>
              <CardContent className="space-y-3">
                ...
                {analysisResult.claude_analysis.breathing_points?.length >
                  0 && (
                  <div>
                    ...
                    <ul className="mt-1 space-y-1">
                      {analysisResult.claude_analysis.breathing_points.map(
                        (bp, i) => (
                          <li key={i} className="text-xs">
                            m.{bp.measure} beat {bp.beat}: {bp.suggestion}
                          </li>
                        ),
                      )}
                    </ul>
                ...
                {analysisResult.claude_analysis.practice_suggestions?.length >
                  0 && (
                  <div>
                    <span className="text-xs font-medium text-[var(--pf-text-secondary)]">
                    ...
                    <ul className="mt-1 space-y-1">
                      {analysisResult.claude_analysis.practice_suggestions.map(
                        (s, i) => (
                          <li key={i} className="text-xs">
                            {s}
                          </li>
