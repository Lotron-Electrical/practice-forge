...
export function AnalyticsPage() {
  ...
  useEffect(() => {
    api
      ...
      .catch(() => {});
    api
      .getAnalyticsTimeByCategory(period)
      .then((d) => setTimeData(d))
      .catch(() => {});
  }, [period]);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((d) => setStalled(d as any[]))
      .catch(() => {});
  }, []);

  useEffect(() => {
    api
      .getSessionHistory({ page: historyPage })
      .then((d) => setHistory(d))
      .catch(() => {});
  }, [historyPage]);
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <h2 className="text-base font-semibold">Time Distribution</h2>
          </CardHeader>
          <CardContent>
            ...
            <CardContent>
              {stalled.length === 0 ? (
                <p className="text-sm text-[var(--pf-text-secondary)] text-center py-6">
                  All pieces are progressing nicely.
                </p>
              ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-[var(--pf-text-secondary)]">
                          ...
                          {s.total_minutes}m invested, {s.days_since_change}d
                          since progress
                        </div>
                      </div>
                      <Clock
                        size={14}
                        className="flex-shrink-0 mt-0.5 text-[var(--pf-text-secondary)]"
                      />
                    </Link>
                  ))}
                </div>
