...
export function ExercisesPage() {
  ...
  const [form, setForm] = useState({
    title: "",
    source: "",
    source_type: "manual" as ExerciseSourceType,
    category_id: "",
    key: "",
    difficulty: "",
    description: "",
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 mb-6">
        ...
        <select
          ...
          className="px-3 py-2 rounded-pf-sm border border-[var(--pf-border-color)] bg-[var(--pf-bg-input)] text-sm"
        >
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input
                ...
                placeholder="e.g. Taffanel & Gaubert No. 4"
              />
              <Input
                label="Source"
                value={form.source}
                ...
                placeholder="e.g. 17 Grands Exercices Journaliers"
              />
              <Select
                label="Source type"
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    source_type: e.target.value as ExerciseSourceType,
                  }))
                }
              >
                <option value="manual">Manual</option>
            ...
            <Input
              ...
              placeholder="e.g. warm-up, daily, scales"
            />
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-[var(--pf-text-secondary)]">
              No exercises found. Add your first exercise or adjust filters.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {exercises.map((ex) => {
            ...
            return (
              <Card
                key={ex.id}
                className="hover:shadow-pf-lg transition-shadow group"
              >
                <CardContent>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    {cat && (
                      <Badge color="var(--pf-accent-teal)">{cat.name}</Badge>
                    )}
                    {ex.key && (
                      <Badge color="var(--pf-accent-lavender)">{ex.key}</Badge>
                    )}
                    <Badge color="var(--pf-text-secondary)">
                      {ex.source_type}
                    </Badge>
                  </div>
                  <DifficultyDots value={ex.difficulty} />
                  {ex.description && (
                    <p className="text-xs text-[var(--pf-text-secondary)] mt-2 line-clamp-2">
                      {ex.description}
                    </p>
                  )}
                  {tags.length > 0 && (
                    <div className="flex gap-1 mt-2 flex-wrap">
                      {tags.map((t: string) => (
                        <span
                          key={t}
                          className="text-xs px-1.5 py-0.5 rounded bg-[var(--pf-bg-hover)] text-[var(--pf-text-secondary)]"
                        >
                          {t}
                        </span>
                      ))}
