...
export function PieceDetailPage() {
  ...
  const [generateForDemand, setGenerateForDemand] = useState<{
    id: string;
    description: string;
  } | null>(null);
  ...
  const load = useCallback(() => {
    ...
    api
      .getTaxonomy()
      .then((d) => setCategories(d as TaxonomyCategory[]))
      .catch(() => {});
    if (id) {
      api
        .getFiles({ linked_type: "piece", linked_id: id })
        .then((d) => {
        ...
        .catch(() => {});
      api
        .getRecordings({ linked_type: "piece", linked_id: id })
        .then((d) => {
          const recs = d as unknown[];
          setRecordingCount(recs.length);
          if (!countsLoadedRef.current && recs.length > 0)
            setRecordingsOpen(true);
        })
        .catch(() => {});
      api
        .getResources({ linked_type: "piece", linked_id: id })
        .then((d) => {
          const res = d as unknown[];
          setResourceCount(res.length);
          if (!countsLoadedRef.current && res.length > 0)
  ...
  if (!piece)
    return <div className="text-[var(--pf-text-secondary)]">Loading...</div>;
  ...
  const saveEdit = async () => {
    await api.updatePiece(piece.id, {
      ...form,
  ...
  const addSection = async () => {
    await api.createSection(piece.id, {
      ...sectionForm,
      sort_order: piece.sections.length,
    });
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-[var(--pf-text-secondary)] mb-4">
        <Link to="/pieces" className="hover:text-[var(--pf-text-primary)]">
          Pieces
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        status: e.target.value as PieceStatus,
                      }))
                    }
                  >
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, general_notes: e.target.value }))
                  }
                />
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      const sheetMusic = linkedFiles.find(
                        (f) =>
                          f.file_type === "sheet_music_digital" ||
                          f.file_type === "sheet_music_scanned",
                      );
            ...
            <CardContent>
              {piece.technical_demands.length === 0 && !showDemandForm && (
                <p className="text-sm text-[var(--pf-text-secondary)]">
                  No tricky spots yet. Mark the hard passages so you can track
                  and target them in practice.
                </p>
              )}
              <div className="space-y-3">
                {piece.technical_demands.map((td) => {
                  ...
                  return (
                    <div
                      key={td.id}
                      className="flex items-start gap-3 py-2 border-b border-[var(--pf-border-color)] last:border-0 flex-wrap sm:flex-nowrap"
                    >
                      <div
                        className="w-1 h-full min-h-[40px] rounded-full"
                        style={{ backgroundColor: "var(--pf-accent-teal)" }}
                      />
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {cat && (
                            <Badge color="var(--pf-accent-teal)">
                              {cat.name}
                            </Badge>
                          )}
                          {td.difficulty && (
                      ...
                      {(!td.linked_exercises ||
                        ...
                        <button
                          onClick={() =>
                            setGenerateForDemand({
                              id: td.id,
                              description: td.description,
                            })
                          }
                          className="p-2.5 sm:p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-accent-gold)]"
                          title="Generate exercise"
                          aria-label="Generate exercise"
                        >
                          <Wand2 size={14} />
                        </button>
                      )}
                      <button
                        onClick={() => deleteDemand(td.id)}
                        className="p-2.5 sm:p-1 text-[var(--pf-text-secondary)] hover:text-[var(--pf-status-needs-work)]"
                        aria-label="Delete tricky spot"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
              ...
              {showDemandForm && (
                <div className="mt-4 p-4 border border-[var(--pf-border-color)] rounded-pf space-y-3">
                  <Input
                    ...
                    placeholder="e.g. Scale in thirds — G major"
                  />
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <Select
                      label="Category"
                      value={demandForm.category_id}
                      onChange={(e) =>
                        setDemandForm((f) => ({
                          ...f,
                    ...
                    <Input
                      label="Bar range"
                      value={demandForm.bar_range}
                      onChange={(e) =>
                        setDemandForm((f) => ({
                          ...f,
                          bar_range: e.target.value,
                        }))
                      }
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      size="sm"
                      onClick={addDemand}
                      disabled={!demandForm.description.trim()}
                    >
                      Add
                    </Button>
                  </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <h2 className="text-base font-semibold">Piece Details</h2>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
              ...
              {piece.target_date && (
                <div className="flex justify-between">
                  ...
                  <span>
                    {new Date(piece.target_date).toLocaleDateString()}{" "}
                    {daysUntil != null && `(${daysUntil}d)`}
                  </span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-[var(--pf-text-secondary)]">Status</span>
                <StatusIndicator {...statusConf} size="sm" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              {piece.sections.length === 0 && !showSectionForm && (
                <p className="text-sm text-[var(--pf-text-secondary)]">
                  No sections yet. Break your piece into passages to track
                    ...
                    <div key={s.id} className="flex items-center gap-2 group">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateSectionStatus(
                            s.id,
                            e.target.value as SectionStatus,
                          )
                        }
              ...
              {showSectionForm && (
                <div className="mt-3 p-3 border border-[var(--pf-border-color)] rounded-pf-sm space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Bar range (e.g. 1-67)"
                  />
                  <div className="flex gap-2 justify-end">
                    <Button
                      variant="secondary"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {!recordingsOpen &&
                    recordingCount != null &&
                    recordingCount > 0 && (
                      <Badge color="var(--pf-accent-gold)">
                        {recordingCount}
                      </Badge>
                    )}
                </div>
                <ChevronDown
                  size={16}
                  className={`text-[var(--pf-text-secondary)] transition-transform ${recordingsOpen ? "rotate-180" : ""}`}
                />
              </button>
            </CardHeader>
            {recordingsOpen && (
          ...
          <Card>
            <CardHeader>
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={`text-[var(--pf-text-secondary)] transition-transform ${resourcesOpen ? "rotate-180" : ""}`}
                />
              </button>
            </CardHeader>
            {resourcesOpen && (
          ...
          <Card>
            <CardHeader>
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={`text-[var(--pf-text-secondary)] transition-transform ${filesOpen ? "rotate-180" : ""}`}
                />
              </button>
            </CardHeader>
            {filesOpen && (
