...
export function DashboardPage() {
  ...
  const showExcerptHelp =
    experienceLevel === "beginner" || experienceLevel === "intermediate";
  const [sessionExplainerDismissed, setSessionExplainerDismissed] = useState(
    () => localStorage.getItem("pf-dismissed-session-explainer") === "true",
  );
  ...
  useEffect(() => {
    ...
    api
      .getTodayRotation()
      .then((d) => setRotation(d as RotationEntry[]))
      .catch(() => {});
    api
      .getExcerpts()
    ...
    api
      ...
      .then((d: any) => {
        if (d?.timeAllocation)
          setTimeAlloc((prev) => ({ ...prev, ...d.timeAllocation }));
      })
      .catch(() => {});
  }, []);
  ...
  for (const p of activePieces) {
    if (p.target_date) {
      ...
      if (days <= 7 && days >= 0) {
        const notReady = p.sections.filter(
          (s) => s.status !== "polished" && s.status !== "solid",
        );
        if (notReady.length > 0) {
          alerts.push({
            color: "var(--pf-status-needs-work)",
            text: `${p.title} target in ${days} days — ${notReady.length} section${notReady.length > 1 ? "s" : ""} still need work`,
          });
        }
      }
    }
  ...
  const staleExcerpts = excerpts.filter((e) => {
    ...
    const days = Math.ceil(
      (Date.now() - new Date(e.last_practiced).getTime()) / 86400000,
    );
    return days > 20;
  });
  if (staleExcerpts.length > 0) {
    alerts.push({
      ...
      text: `${staleExcerpts.length} excerpt${staleExcerpts.length > 1 ? "s" : ""} not practiced in 20+ days`,
    });
  }
  ...
  const excerptCounts = {
    audition_ready: excerpts.filter((e) => e.status === "audition_ready")
      .length,
    ...
    <div>
          ...
          <Card className="mb-6">
            <CardContent className="py-8">
              <h2 className="text-xl font-semibold text-center mb-2">
                Welcome to Practice Forge
              </h2>
              <p className="text-sm text-[var(--pf-text-secondary)] text-center mb-6 max-w-md mx-auto">
                Get started in three steps — add your music, then let the app
                plan your practice sessions.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-lg mx-auto">
                <Link
                  to="/pieces"
                  className="flex flex-col items-center gap-2 p-4 rounded-pf border border-[var(--pf-border-color)] hover:border-[var(--pf-accent-gold)] transition-colors"
                >
                  ...
                  <span className="text-xs text-[var(--pf-text-secondary)]">
                    A song or work you're learning
                  </span>
                </Link>
                <Link
                  to="/session"
                  className="flex flex-col items-center gap-2 p-4 rounded-pf border border-[var(--pf-border-color)] hover:border-[var(--pf-accent-teal)] transition-colors"
                >
                  ...
                  <span className="text-xs text-[var(--pf-text-secondary)]">
                    AI plans your practice time
                  </span>
                </Link>
                <Link
                  to="/tutorial"
                  className="flex flex-col items-center gap-2 p-4 rounded-pf border border-[var(--pf-border-color)] hover:border-[var(--pf-accent-lavender)] transition-colors"
                >
                  <BookOpen
                    size={24}
                    style={{ color: "var(--pf-accent-lavender)" }}
                  />
      ...
      {experienceLevel === "beginner" && !sessionExplainerDismissed && (
        <Card
          className="mb-6"
          style={{
            borderColor: "var(--pf-accent-teal)",
            borderWidth: "1px",
            borderStyle: "solid",
            backgroundColor:
              "color-mix(in srgb, var(--pf-accent-teal) 5%, var(--pf-bg-primary))",
          }}
        >
          <CardContent>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ol className="text-xs text-[var(--pf-text-secondary)] space-y-1 list-decimal list-inside">
                    ...
                    <li>
                      The app builds a structured plan with warm-up, technique,
                      and repertoire
                    </li>
                  ...
                  <p className="text-xs text-[var(--pf-text-secondary)] mt-2">
                    Time allocation is based on your pieces and settings. You
                    can customise it in Settings.
                  </p>
                </div>
              </div>
              <button
                onClick={() => {
                  localStorage.setItem(
                    "pf-dismissed-session-explainer",
                    "true",
                  );
          ...
          <CardContent className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-[var(--pf-text-secondary)]">
                {activePieces.length} active piece
                {activePieces.length !== 1 ? "s" : ""} &middot;{" "}
                {rotation.length} excerpt{rotation.length !== 1 ? "s" : ""} in
                rotation
              </p>
              ...
              <div className="flex gap-0.5 mt-3 h-2 w-48 rounded-full overflow-hidden">
                ...
                <div
                  className="h-full"
                  style={{
                    flex: timeAlloc.repertoire,
                    backgroundColor: "var(--pf-status-in-progress)",
                  }}
                />
                <div
                  className="h-full"
                  style={{
                    flex: timeAlloc.excerpts,
                    backgroundColor: "var(--pf-accent-lavender)",
                  }}
                />
                <div
                  className="h-full"
                  style={{
                    flex: timeAlloc.buffer,
                    backgroundColor: "var(--pf-text-secondary)",
                  }}
                />
              </div>
              <p className="text-xs text-[var(--pf-text-secondary)] mt-1">
                Warm-up → Fundamentals → Technique → Your pieces → Excerpts
              </p>
            </div>
      ...
      <div className="mb-6">
        {!quickLogOpen ? (
          ...
          >
            <PenLine size={16} style={{ color: "var(--pf-accent-gold)" }} /> Log
            Practice
          </button>
        ) : (
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[var(--pf-text-secondary)] hover:text-[var(--pf-text-primary)]"
                >
                  <ChevronUp size={18} />
                </button>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                      <label className="text-sm text-[var(--pf-text-secondary)]">
                        Duration
                      </label>
                      <input
                        type="number"
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await api.quickLog({
                            notes: qlNotes.trim(),
                            duration_min: qlDuration,
                            rating: qlRating || undefined,
                          });
                          setQlNotes("");
                          setQlDuration(30);
                          setQlRating(null);
                    ...
                    >
                      <CheckCircle size={14} />{" "}
                      {qlSubmitting ? "Logging..." : "Log Practice"}
                    </Button>
                  </div>
                </div>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            ...
            <CardContent>
              {activePieces.length === 0 ? (
                <p className="text-sm text-[var(--pf-text-secondary)]">
                  ...
                  to see your repertoire and progress here.
                </p>
              ) : (
                <div className="space-y-3">
                  {activePieces.slice(0, 5).map((piece) => {
                    ...
                    const daysUntil = piece.target_date
                      ? Math.ceil(
                          (new Date(piece.target_date).getTime() - Date.now()) /
                            86400000,
                        )
                      : null;
                    return (
                      <Link
                        key={piece.id}
                        to={`/pieces/${piece.id}`}
                        className="flex items-center gap-3 py-2 hover:bg-[var(--pf-bg-hover)] rounded-pf-sm px-2 -mx-2 transition-colors"
                      >
                        <Badge
                          color={
                            piece.priority === "high"
                                ...
                                s.status === "polished"
                                  ...
                                  : s.status === "solid"
                                    ...
                                    : s.status === "working_on"
                                      ...
                                      : "--pf-status-not-started";
                              return (
                                <div
                                  key={s.id}
                                  className="flex-1 h-2 rounded-full"
                                  style={{ backgroundColor: `var(${cv})` }}
                                />
                              );
                            })}
                          </div>
                        ...
                        {daysUntil != null && (
                          <span
                            className={`text-xs font-medium ${daysUntil <= 7 ? "text-[var(--pf-status-needs-work)]" : "text-[var(--pf-text-secondary)]"}`}
                          >
        ...
        <Card>
              ...
              <div className="flex items-center gap-1.5">
                ...
                {showExcerptHelp && (
                  <div className="relative group">
                    <Info
                      size={14}
                      className="text-[var(--pf-text-secondary)] cursor-help"
                    />
                    <div className="invisible group-hover:visible absolute left-1/2 -translate-x-1/2 top-full mt-1.5 z-10 w-[250px] px-3 py-2 rounded-pf bg-[var(--pf-bg-secondary)] border border-[var(--pf-border-color)] shadow-lg text-xs text-[var(--pf-text-secondary)] leading-relaxed">
                      Excerpts are short orchestral passages practiced regularly
                      for auditions and orchestra preparation.
                    </div>
                  </div>
                )}
              </div>
          ...
          <CardContent>
            {excerpts.length === 0 ? (
              <p className="text-sm text-[var(--pf-text-secondary)]">
                No excerpts yet. Add orchestral excerpts to track your audition
                prep and keep them in rotation.
              </p>
            ) : (
              <>
                ...
                <div className="flex items-center justify-center mb-4">
                  ...
                  <span className="text-sm text-[var(--pf-text-secondary)] ml-2">
                    total
                  </span>
                </div>
                <div className="space-y-1.5 text-sm">
                  {Object.entries(excerptCounts).map(([status, count]) => {
                    ...
                    const conf =
                      EXCERPT_STATUS_CONFIG[
                        status as keyof typeof EXCERPT_STATUS_CONFIG
                      ];
                    return (
                      <div key={status} className="flex items-center gap-2">
                        <div
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: `var(${conf.colorVar})` }}
                        />
                ...
                {rotation.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-[var(--pf-border-color)]">
                    <div className="flex items-center gap-1.5 mb-2">
                      <h3 className="text-xs font-semibold text-[var(--pf-text-secondary)]">
                        TODAY'S EXCERPTS
                      </h3>
                      {showExcerptHelp && (
                        <div className="relative group">
                          ...
                          />
                          <div className="invisible group-hover:visible absolute left-1/2 -translate-x-1/2 top-full mt-1.5 z-10 w-[250px] px-3 py-2 rounded-pf bg-[var(--pf-bg-secondary)] border border-[var(--pf-border-color)] shadow-lg text-xs text-[var(--pf-text-secondary)] leading-relaxed">
                            These are the excerpts selected for today's
                            rotation. Practise each one to keep your audition
                            repertoire fresh.
                          </div>
                        </div>
                      )}
                    ...
                    {rotation.map((r) => {
                      const sConf =
                        EXCERPT_STATUS_CONFIG[
                          r.status as keyof typeof EXCERPT_STATUS_CONFIG
                        ] || EXCERPT_STATUS_CONFIG.needs_work;
                      return (
                        <div
                          key={r.id}
                        ...
                        >
                          <div
                            ...
                            style={{
                              backgroundColor: `var(${sConf.colorVar})`,
                            }}
                          />
      ...
      <MilestoneToast
        ...
        totalSessions={stats.weekSessions}
        streak={stats.streak}
        totalHours={totalHours}
        hasSolidPiece={pieces.some((p) => p.status === "performance_ready")}
      />
      ...
      {alerts.length > 0 && (
        <Card>
          <CardHeader>
            <h2 className="text-base font-semibold">Alerts & Nudges</h2>
          </CardHeader>
