...
> = {
  piece_audit: {
    ...
    description:
      "Record a full play-through and get bar-by-bar accuracy results",
  },
  excerpt_spot_check: {
    icon: ListMusic,
    label: "Excerpt Spot-Check",
    color: "var(--pf-accent-lavender)",
    description: "Play 5-10 random excerpts cold to test audition readiness",
  },
  technique_assessment: {
    icon: BookOpen,
    label: "Technique Assessment",
    color: "var(--pf-accent-teal)",
    description: "Evaluate skills by category with generated exercises",
  },
  weekly_review: {
    icon: Calendar,
    label: "Weekly Review",
    color: "var(--pf-status-in-progress)",
    description: "Automated summary of your practice week",
  },
};
...
export function AssessmentsPage() {
  ...
  const load = useCallback(() => {
    api
      .getAssessments()
      .then((d) => setAssessments(d as Assessment[]))
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    api
      .getPieces()
      .then((d) =>
        setPieces((d as Piece[]).filter((p) => p.status !== "archived")),
      )
      .catch(() => {});
  }, [load]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="hover:shadow-pf-lg transition-shadow">
          <CardContent className="space-y-3">
            ...
            <p className="text-xs text-[var(--pf-text-secondary)]">
              Record a play-through with bar-by-bar scoring
            </p>
            <select
              value={selectedPiece}
        ...
        <Card className="hover:shadow-pf-lg transition-shadow">
          <CardContent className="space-y-3">
            ...
            <p className="text-xs text-[var(--pf-text-secondary)]">
              Category-by-category skill evaluation
            </p>
        ...
        <Card className="hover:shadow-pf-lg transition-shadow">
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2">
              <Calendar
                size={18}
                style={{ color: "var(--pf-status-in-progress)" }}
              />
              <h3 className="text-sm font-semibold">Weekly Review</h3>
            </div>
            <p className="text-xs text-[var(--pf-text-secondary)]">
              Auto-generated practice summary
            </p>
            <Button
              size="sm"
              variant="secondary"
          ...
          {assessments.length === 0 ? (
            ...
            <div className="space-y-2">
              {assessments.map((a) => {
                ...
                const piece = a.piece_id
                  ? pieces.find((p) => p.id === a.piece_id)
                  : null;

                return (
                  <div
                    key={a.id}
                    className="border border-[var(--pf-border-color)] rounded-pf overflow-hidden"
                  >
                    <button
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium">
                          {conf.label}
                          {piece && (
                            <span className="text-[var(--pf-text-secondary)] font-normal">
                              {" "}
                              — {piece.title}
                            </span>
                          )}
                      ...
                      {a.overall_rating && (
                        <Badge color={RATING_COLORS[a.overall_rating]}>
                          {a.overall_rating.replace("_", " ")}
                        </Badge>
                      )}
                    ...
                    {isExpanded && (
                      <div className="p-4 pt-0 border-t border-[var(--pf-border-color)]">
                        {a.type === "weekly_review" && a.results && (
                          <WeeklyReviewCard
                            data={a.results as unknown as WeeklyReviewData}
                          />
                        )}
                        ...
                        {a.notes && (
                          <p className="text-xs text-[var(--pf-text-secondary)] mt-2 italic">
                            "{a.notes}"
                          </p>
                        )}
                      </div>
  ...
  return (
    <div className="pt-3 space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
        ...
        <div>
          <div className="text-xl font-bold">
            {data.avg_pitch_accuracy != null
              ? `${data.avg_pitch_accuracy}%`
              : "—"}
          </div>
          <div className="text-xs text-[var(--pf-text-secondary)]">
            Avg pitch accuracy
      ...
      {Object.keys(data.category_breakdown).length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-[var(--pf-text-secondary)] mb-1">
            Time by category
          </h4>
          <div className="space-y-1">
            {Object.entries(data.category_breakdown).map(([cat, min]) => (
              <div key={cat} className="flex items-center gap-2 text-xs">
                <span className="w-24 text-[var(--pf-text-secondary)] capitalize">
                  {cat}
                </span>
                <div className="flex-1 h-2 bg-[var(--pf-bg-hover)] rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full bg-[var(--pf-accent-teal)]"
                    style={{
                      width: `${Math.min(100, (min / data.total_practice_minutes) * 100)}%`,
                    }}
                  />
                </div>
...
function SpotCheckCard({
  ...
  return (
    <div className="pt-3">
      <h4 className="text-xs font-semibold text-[var(--pf-text-secondary)] mb-2">
        Excerpts to play
      </h4>
      <div className="space-y-1">
        {results.excerpts.map((ex, i) => (
          <div key={ex.id} className="flex items-center gap-2 text-sm">
            <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold bg-[var(--pf-bg-hover)]">
              {i + 1}
            </span>
