...
import type { TierDefinition, SubscriptionTier, SubscriptionInfo } from '../core/types';
...
export function PricingPage() {
  ...
  useEffect(() => {
    api.getTiers().then((data: { tiers: TierDefinition[] }) => setTiers(data.tiers)).catch(() => {});
    api.getSubscription().then((data: SubscriptionInfo) => setCurrentTier(data.tier)).catch(() => {});
  }, []);
              ...
              <div className="mb-4">
                ...
                {tier.per_student_monthly && (
                  <p className="text-xs text-[var(--pf-text-secondary)] mt-1">
                    + ${interval === 'monthly' ? tier.per_student_monthly : tier.per_student_annual}/student/{interval === 'monthly' ? 'mo' : 'yr'}
                  </p>
                )}
