...
export function detectFileType(mimetype, ext) {
  ext = ext.toLowerCase();
  if (['.xml', '.mxl', '.mei', '.abc'].includes(ext)) return 'sheet_music_digital';
  if (['.pdf', '.png', '.jpg', '.jpeg', '.tiff'].includes(ext)) return 'sheet_music_scanned';
...
export function initialProcessingStatus(fileType) {
  if (fileType === 'sheet_music_scanned' || fileType === 'video') return 'pending';
  return 'complete';
}
...
export function moveToTypeDir(currentPath, fileType, filename) {
  ...
  else if (fileType === 'sheet_music_digital') targetDir = path.join(DATA_DIR, 'scores');
