...
router.get('/', async (req, res) => {
  ...
  if (category_id) { n++; sql += ` AND e.category_id = $${n}`; params.push(category_id); }
  if (source_type) { n++; sql += ` AND e.source_type = $${n}`; params.push(source_type); }
  if (difficulty_min) { n++; sql += ` AND e.difficulty >= $${n}`; params.push(Number(difficulty_min)); }
  if (difficulty_max) { n++; sql += ` AND e.difficulty <= $${n}`; params.push(Number(difficulty_max)); }
  if (keyFilter) { n++; sql += ` AND e.key = $${n}`; params.push(keyFilter); }
  if (search) { const s = `%${search}%`; sql += ` AND (e.title ILIKE $${n+1} OR e.description ILIKE $${n+2} OR e.source ILIKE $${n+3})`; params.push(s, s, s); n += 3; }
...
router.put('/:id', async (req, res) => {
  ...
  await execute(
    ...
    [
      ...
      secondary_categories ? JSON.stringify(secondary_categories) : existing.secondary_categories,
