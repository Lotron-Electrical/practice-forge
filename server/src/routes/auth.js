...
router.post('/register', asyncHandler(async (req, res) => {
  const { email, password, display_name, instrument = 'Flute', level = 'student' } = req.body;
  ...
  await execute(
    `INSERT INTO users (id, email, password_hash, display_name, instrument, level)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [id, email.toLowerCase(), passwordHash, display_name || email.split('@')[0], instrument, level]
  );

  const user = await queryOne('SELECT id, email, display_name, instrument, level, created_at FROM users WHERE id = $1', [id]);
...
router.put('/me', requireAuth, asyncHandler(async (req, res) => {
  ...
  const { display_name, instrument, level, institution, bio, privacy_settings } = req.body;
  ...
  await execute(
    ...
    [
      ...
      privacy_settings ? JSON.stringify(privacy_settings) : user.privacy_settings,
      req.user.id,
    ]
  );
...
router.put('/me/password', requireAuth, asyncHandler(async (req, res) => {
  ...
  await execute('UPDATE users SET password_hash = $1, updated_at = NOW() WHERE id = $2', [newHash, req.user.id]);
