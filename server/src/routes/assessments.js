...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(
      await queryOne("SELECT * FROM assessments WHERE id = $1", [
        req.params.id,
      ]),
    );
  }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await execute(
     ...
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [
        ...
        sort_order,
        score ?? null,
        bar_results ? JSON.stringify(bar_results) : null,
      ],
    );
    ...
    res
      .status(201)
      .json(
        await queryOne("SELECT * FROM assessment_recordings WHERE id = $1", [
          id,
        ]),
      );
  }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (excerpts.length === 0)
      return res.status(400).json({ error: "No excerpts in library" });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const totalMin = sessions.reduce(
      (s, sess) =>
        s + (sess.actual_duration_min || sess.planned_duration_min || 0),
      0,
    );
    ...
    if (sessionIds.length > 0) {
      ...
      categoryBreakdown = Object.fromEntries(
        blocks.map((b) => [b.category, parseInt(b.total_min)]),
      );
    }
    ...
    const results = {
      ...
      status_changes: {
        pieces: recentPieceChanges,
        sections: recentSectionChanges,
      },
      ...
      avg_pitch_accuracy:
        recordings.length > 0
          ? +(
              recordings
                .filter((r) => r.pitch_accuracy_pct != null)
                .reduce((s, r) => s + r.pitch_accuracy_pct, 0) /
              Math.max(
                1,
                recordings.filter((r) => r.pitch_accuracy_pct != null).length,
              )
            ).toFixed(1)
          : null,
    };
    ...
    res
      .status(201)
      .json(await queryOne("SELECT * FROM assessments WHERE id = $1", [id]));
  }),
);
