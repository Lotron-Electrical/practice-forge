...
router.get('/:id/community', asyncHandler(async (req, res) => {
...
router.post('/:id/rate', asyncHandler(async (req, res) => {
  ...
  if (!difficulty_rating || difficulty_rating < 1 || difficulty_rating > 10) {
    return res.status(400).json({ error: 'difficulty_rating must be between 1 and 10' });
  }
...
router.post('/:id/notes', asyncHandler(async (req, res) => {
  ...
  const created = await queryOne('SELECT * FROM excerpt_community_notes WHERE id = $1', [id]);
  res.status(201).json(created);
}));

// DELETE /notes/:noteId — Delete own note only
router.delete('/notes/:noteId', asyncHandler(async (req, res) => {
  const note = await queryOne('SELECT * FROM excerpt_community_notes WHERE id = $1', [req.params.noteId]);
  if (!note) return res.status(404).json({ error: 'Note not found' });
  if (note.user_id !== req.user.id) return res.status(403).json({ error: 'Not authorized' });

  await execute('DELETE FROM excerpt_community_notes WHERE id = $1', [req.params.noteId]);
  res.json({ deleted: true });
}));

// POST /notes/:noteId/upvote — Increment upvotes
router.post('/notes/:noteId/upvote', asyncHandler(async (req, res) => {
  const note = await queryOne('SELECT id FROM excerpt_community_notes WHERE id = $1', [req.params.noteId]);
  if (!note) return res.status(404).json({ error: 'Note not found' });

  await execute('UPDATE excerpt_community_notes SET upvotes = upvotes + 1 WHERE id = $1', [req.params.noteId]);
  const updated = await queryOne('SELECT * FROM excerpt_community_notes WHERE id = $1', [req.params.noteId]);
  res.json(updated);
}));
