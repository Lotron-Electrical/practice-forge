    ...
    try {
      ...
      await execute(
        ...
        [
          ...
          result.extracted_composer,
        ],
      );

      await execute(
        "UPDATE uploaded_files SET processing_status = $1 WHERE id = $2",
        ["needs_review", file.id],
      );

      const omrResult = await queryOne(
        "SELECT * FROM omr_results WHERE id = $1",
        [omrId],
      );
      res.json(omrResult);
    } catch (omrErr) {
  ...
  try {
    ...
    if (file.file_type === "sheet_music_digital") {
      ...
      if (!fs.existsSync(xmlPath))
        return res
          .status(404)
          .json({ error: "MusicXML file not found on disk" });
    ...
    await execute(
      ...
      [
        ...
        result.difficulty_estimate,
        result.register_low,
        result.register_high,
        result.total_measures,
    ...
    const analysisResult = await queryOne(
      "SELECT * FROM analysis_results WHERE id = $1",
      [analysisId],
    );
    const demands = await queryAll(
      "SELECT * FROM analysis_demands WHERE analysis_id = $1",
      [analysisId],
    );
...
router.post("/trigger-claude/:analysisId", async (req, res) => {
  try {
    ...
    if (file.file_type === "sheet_music_digital") {
      ...
      if (!omr || !omr.musicxml_path)
        return res.status(400).json({ error: "No MusicXML available" });
      musicxmlContent = fs.readFileSync(
        path.resolve(DATA_DIR, omr.musicxml_path),
        "utf-8",
      );
    }
...
router.post("/demands/:demandId/import", async (req, res) => {
  try {
    const demand = await queryOne(
      "SELECT * FROM analysis_demands WHERE id = $1",
      [req.params.demandId],
    );
    ...
    const newDemand = await queryOne(
