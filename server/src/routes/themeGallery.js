...
router.post('/', asyncHandler(async (req, res) => {
  ...
  await execute(
    ...
    [id, req.user.id, name.trim(), description || '', base_theme || 'light', JSON.stringify(tokens), JSON.stringify(tags || [])]
  );
  const theme = await queryOne('SELECT * FROM community_themes WHERE id = $1', [id]);
  res.status(201).json(theme);
}));

// PUT /:id — Update theme (owner only)
router.put('/:id', asyncHandler(async (req, res) => {
  const existing = await queryOne('SELECT * FROM community_themes WHERE id = $1', [req.params.id]);
  ...
  const updated = await queryOne('SELECT * FROM community_themes WHERE id = $1', [req.params.id]);
  res.json(updated);
}));
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  await execute('DELETE FROM community_themes WHERE id = $1', [req.params.id]);
  res.json({ deleted: true });
}));

// POST /:id/favorite — Toggle favorite
router.post('/:id/favorite', asyncHandler(async (req, res) => {
  const theme = await queryOne('SELECT id FROM community_themes WHERE id = $1', [req.params.id]);
  if (!theme) return res.status(404).json({ error: 'Theme not found' });
  ...
  if (existing) {
    await execute('DELETE FROM theme_favorites WHERE user_id = $1 AND theme_id = $2', [req.user.id, req.params.id]);
    await execute('UPDATE community_themes SET favorites_count = GREATEST(favorites_count - 1, 0) WHERE id = $1', [req.params.id]);
    ...
    await execute('UPDATE community_themes SET favorites_count = favorites_count + 1 WHERE id = $1', [req.params.id]);
...
router.post('/:id/download', asyncHandler(async (req, res) => {
  await execute('UPDATE community_themes SET downloads_count = downloads_count + 1 WHERE id = $1', [req.params.id]);
  const theme = await queryOne('SELECT * FROM community_themes WHERE id = $1', [req.params.id]);
  if (!theme) return res.status(404).json({ error: 'Theme not found' });
  res.json(theme);
}));
