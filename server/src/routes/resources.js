...
router.get('/', async (req, res) => {
  ...
  if (!linked_type || !linked_id) return res.status(400).json({ error: 'linked_type and linked_id required' });
...
router.post('/', async (req, res) => {
  const { linked_type, linked_id, resource_type, title, url, source, description = '', thumbnail_url, attribution = '' } = req.body;
  if (!linked_type || !linked_id || !resource_type || !title || !url || !source) {
    return res.status(400).json({ error: 'linked_type, linked_id, resource_type, title, url, source required' });
  }
  const id = uuid();
  await execute(
    ...
    [id, linked_type, linked_id, resource_type, title, url, source, description, thumbnail_url || null, attribution]
  );
  res.status(201).json(await queryOne('SELECT * FROM resources WHERE id = $1', [id]));
});

// Delete a resource
router.delete('/:id', async (req, res) => {
  if (!(await queryOne('SELECT id FROM resources WHERE id = $1', [req.params.id])))
...
router.get('/search/imslp', async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ error: 'IMSLP search failed', details: err.message });
  }
});
...
router.get('/search/wikipedia', async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ error: 'Wikipedia search failed', details: err.message });
  }
});
...
router.get('/search/youtube', async (req, res) => {
  ...
  if (apiKey) {
    try {
      ...
      return res.status(502).json({ error: 'YouTube search failed', details: err.message });
    }
  }
...
router.post('/auto-discover', async (req, res) => {
  ...
  try {
    ...
    const [imslp, wikipedia, youtube] = await Promise.all([
      fetch(`${base}/api/resources/search/imslp?q=${encodeURIComponent(query)}`).then(r => r.json()).catch(() => []),
      fetch(`${base}/api/resources/search/wikipedia?q=${encodeURIComponent(query)}`).then(r => r.json()).catch(() => []),
      fetch(`${base}/api/resources/search/youtube?q=${encodeURIComponent(query)}`).then(r => r.json()).catch(() => []),
    ]);
    res.json({ imslp, wikipedia, youtube });
  } catch (err) {
    res.status(500).json({ error: 'Auto-discover failed', details: err.message });
  }
});
