...
router.post('/', asyncHandler(async (req, res) => {
  const { type, content_type, content_id, description, notation_data, deadline, invited_user_ids = [] } = req.body;
  ...
  await execute(
    ...
    [id, req.user.id, type, content_type || null, content_id || null,
     description || null, notation_data ? JSON.stringify(notation_data) : null,
...
router.put('/:id/cancel', asyncHandler(async (req, res) => {
  const challenge = await queryOne('SELECT * FROM challenges WHERE id = $1', [req.params.id]);
  if (!challenge) return res.status(404).json({ error: 'Challenge not found' });
  if (challenge.creator_id !== req.user.id) return res.status(403).json({ error: 'Only the creator can cancel' });

  await execute("UPDATE challenges SET status = 'cancelled' WHERE id = $1", [req.params.id]);
  res.json(await queryOne('SELECT * FROM challenges WHERE id = $1', [req.params.id]));
}));
...
router.put('/:id/accept', asyncHandler(async (req, res) => {
  ...
  if (!participant) return res.status(404).json({ error: 'No pending invitation found' });
  ...
  if (!pending) {
    await execute("UPDATE challenges SET status = 'active' WHERE id = $1", [req.params.id]);
  }
...
router.put('/:id/decline', asyncHandler(async (req, res) => {
  const participant = await queryOne(
    "SELECT * FROM challenge_participants WHERE challenge_id = $1 AND user_id = $2 AND status = 'invited'",
    [req.params.id, req.user.id]
  );
...
router.put('/:id/submit', asyncHandler(async (req, res) => {
  ...
  if (!participant) return res.status(404).json({ error: 'Not an accepted participant' });
  ...
  if (!remaining) {
    ...
    await execute("UPDATE challenges SET status = 'completed' WHERE id = $1", [req.params.id]);
    ...
    for (const p of ranked) {
      ...
      await execute(
        ...
        [eventId, p.user_id, req.params.id,
         JSON.stringify({ challenge_id: req.params.id, rank: p.rank, score: p.score, display_name: p.display_name })]
      );
    }
  }
...
router.post('/weekly/generate', asyncHandler(async (req, res) => {
  // Pick a random excerpt or scale as content
  const excerpt = await queryOne('SELECT * FROM excerpts ORDER BY RANDOM() LIMIT 1');
  ...
  await execute(
    ...
    [id, req.user.id, contentType, contentId, description, deadline.toISOString()]
  );
