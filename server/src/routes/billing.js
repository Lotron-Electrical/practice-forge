...
import { getUserTier, getAiUsageThisMonth, getTierLimits } from '../middleware/tierLimits.js';
...
function tierFromPriceId(priceId) {
  if (priceId === PRICES.solo_monthly() || priceId === PRICES.solo_annual()) return 'solo';
  if (priceId === PRICES.pro_monthly() || priceId === PRICES.pro_annual()) return 'pro';
  if (priceId === PRICES.teacher_monthly() || priceId === PRICES.teacher_annual()) return 'teacher';
  ...
  res.json({
    tiers: [
      ...
      },
      {
        ...
        price_monthly: 49,
        price_annual: 470,
        per_student_monthly: 3,
        per_student_annual: 29,
        features: [
          'Everything in Pro',
          'Student roster management',
          'Assignments with due dates',
          'Lesson notes & handoff cards',
          'Student insights dashboard',
          'Students get Solo access included',
        ],
        limits: getTierLimits('teacher'),
      },
    ],
  });
});
...
router.post('/create-checkout-session', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!priceId) {
    return res.status(400).json({ error: `Invalid plan: ${tier} ${interval}` });
  }

  const s = getStripe();

  // Get or create Stripe customer
  let sub = await queryOne('SELECT stripe_customer_id FROM subscriptions WHERE user_id = $1', [req.user.id]);
...
router.post('/create-portal-session', requireAuth, asyncHandler(async (req, res) => {
  const sub = await queryOne('SELECT stripe_customer_id FROM subscriptions WHERE user_id = $1', [req.user.id]);
