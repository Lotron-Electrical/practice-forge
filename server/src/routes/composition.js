...
import {
  generateScale, generateArpeggio, generateIntervalDrill,
...
router.post('/generate/rule', async (req, res) => {
  try {
    ...
    if (!type) return res.status(400).json({ error: 'type is required (scale, arpeggio, interval, articulation)' });
...
router.post('/generate/from-demand', async (req, res) => {
  try {
    ...
    let demand;
    if (demand_id) {
      demand = await queryOne('SELECT * FROM technical_demands WHERE id = $1', [demand_id]);
    ...
    if (!process.env.ANTHROPIC_API_KEY) {
      return res.status(400).json({ error: 'Cannot generate: demand does not match rule-based patterns and ANTHROPIC_API_KEY is not configured' });
    }
...
router.post('/generate/save', async (req, res) => {
  try {
    const { title, abc, description, key, difficulty, tags = [], category_id, generation_method, prompt_used, demand_id } = req.body;
    if (!title || !abc) return res.status(400).json({ error: 'title and abc are required' });

    const id = uuid();
    const sourceType = generation_method === 'claude_api' ? 'generated_ai' : 'generated_rule';
    ...
    await execute(
      ...
      [id, title, 'Generated', sourceType, category_id || null, key || null, difficulty || null, description || '', JSON.stringify(tags), abc, 'abc', generationContext]
    );

    // Link to demand if specified
    if (demand_id) {
      const demand = await queryOne('SELECT * FROM technical_demands WHERE id = $1', [demand_id]);
      if (demand) {
        await execute('INSERT INTO demand_exercises (demand_id, exercise_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [demand_id, id]);
      }
    }

    const exercise = await queryOne('SELECT e.*, tc.name as category_name FROM exercises e LEFT JOIN taxonomy_categories tc ON e.category_id = tc.id WHERE e.id = $1', [id]);
...
router.post('/generate/excerpt-prep', async (req, res) => {
  try {
    ...
    const excerpt = await queryOne('SELECT * FROM excerpts WHERE id = $1', [excerpt_id]);
    if (!excerpt) return res.status(404).json({ error: 'Excerpt not found' });

    if (!process.env.ANTHROPIC_API_KEY) {
      return res.status(400).json({ error: 'ANTHROPIC_API_KEY is not configured for AI generation' });
    }
...
router.post('/generate/warmup', async (req, res) => {
  try {
    ...
    let context = {};
    if (session_id) {
      const blocks = await queryAll('SELECT * FROM session_blocks WHERE session_id = $1', [session_id]);
      // Gather keys from linked pieces
      for (const block of blocks) {
        if (block.linked_type === 'piece' && block.linked_id) {
          const piece = await queryOne('SELECT * FROM pieces WHERE id = $1', [block.linked_id]);
          if (piece) {
            // Try to get key from analysis
            const files = await queryAll('SELECT id FROM uploaded_files WHERE linked_type = $1 AND linked_id = $2', ['piece', piece.id]);
            for (const f of files) {
              const analysis = await queryOne('SELECT key_signature FROM analysis_results WHERE file_id = $1 AND status = $2', [f.id, 'complete']);
