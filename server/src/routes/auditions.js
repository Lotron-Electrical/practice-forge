...
router.get('/', asyncHandler(async (req, res) => {
  const rows = await queryAll('SELECT * FROM auditions ORDER BY audition_date DESC');
  for (const r of rows) {
    r.repertoire = typeof r.repertoire === 'string' ? JSON.parse(r.repertoire) : r.repertoire;
  }
  res.json(rows);
}));
...
router.get('/upcoming', asyncHandler(async (req, res) => {
  ...
  for (const a of auditions) {
    a.repertoire = typeof a.repertoire === 'string' ? JSON.parse(a.repertoire) : a.repertoire;
    a.days_until = Math.ceil((new Date(a.audition_date).getTime() - Date.now()) / 86400000);
  }
  ...
  res.json({
    ...
    pieces_with_dates: pieces.map(p => ({
      ...p,
      days_until: Math.ceil((new Date(p.audition_date).getTime() - Date.now()) / 86400000),
    })),
    excerpts_with_dates: excerpts.map(e => ({
      ...e,
      days_until: Math.ceil((new Date(e.audition_date).getTime() - Date.now()) / 86400000),
    })),
  });
}));
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (!title || !audition_date) return res.status(400).json({ error: 'title and audition_date are required' });
  ...
  row.repertoire = typeof row.repertoire === 'string' ? JSON.parse(row.repertoire) : row.repertoire;
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  const existing = await queryOne('SELECT * FROM auditions WHERE id = $1', [req.params.id]);
  ...
  const row = await queryOne('SELECT * FROM auditions WHERE id = $1', [req.params.id]);
  row.repertoire = typeof row.repertoire === 'string' ? JSON.parse(row.repertoire) : row.repertoire;
  res.json(row);
}));
