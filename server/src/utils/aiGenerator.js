...
const SYSTEM_PROMPT = `You are a music composition assistant specialising in flute exercises and practice routines for professional flautists. You generate exercises in ABC notation format.

Rules:
- Flute range: C4 to C7 (in ABC: C to c''')
...
export async function generateWithAI(params) {
  const { type, prompt, context = {} } = params;
...
function buildExcerptPrepPrompt(prompt, context) {
  ...
  parts.push('2. Interval exercises targeting the specific intervals in the passage');
...
function buildWarmupPrompt(prompt, context) {
  const parts = [`Generate a personalised warm-up routine for a practice session.`];
  if (prompt) parts.push(prompt);
  if (context.keys) parts.push(`Session will cover these keys: ${context.keys.join(', ')}`);
  if (context.registers) parts.push(`Register focus: ${context.registers}`);
  if (context.duration_min) parts.push(`Target warm-up duration: ${context.duration_min} minutes`);
  parts.push('\nInclude:');
