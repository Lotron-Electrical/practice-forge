...
describe('utils/ruleGenerator', () => {
  describe('generateScale', () => {
    ...
    it('generates valid ABC notation', () => {
      const result = generateScale({ key: 'G', scaleType: 'major', octaves: 1 });
    ...
    it('supports thirds pattern', () => {
      const result = generateScale({ key: 'C', scaleType: 'major', pattern: 'thirds' });
      expect(result.title).toContain('thirds');
    });

    it('handles chromatic scale type', () => {
      const result = generateScale({ key: 'C', scaleType: 'chromatic', octaves: 1 });
    ...
    it('caps difficulty at 10', () => {
      const result = generateScale({ key: 'C', scaleType: 'chromatic', octaves: 3 });
  ...
  describe('generateIntervalDrill', () => {
    ...
    it('defaults to octave for unknown intervals', () => {
      const result = generateIntervalDrill({ key: 'C', interval: 'unknown_interval' });
  ...
  describe('generateArticulationDrill', () => {
    it('returns correct structure', () => {
      const result = generateArticulationDrill({ key: 'C', articulation: 'staccato' });
    ...
    it('generates legato exercise', () => {
      const result = generateArticulationDrill({ key: 'D', articulation: 'legato' });
      expect(result.title).toContain('legato');
      expect(result.tags).toContain('legato');
    });

    it('handles double tongue articulation', () => {
      const result = generateArticulationDrill({ key: 'C', articulation: 'double_tongue' });
      expect(result.difficulty).toBe(6);
      expect(result.tags).toContain('double_tongue');
    });

    it('has higher difficulty for tonguing exercises', () => {
      const staccato = generateArticulationDrill({ key: 'C', articulation: 'staccato' });
      const doubleTongue = generateArticulationDrill({ key: 'C', articulation: 'double_tongue' });
  ...
  describe('generateFromDemand', () => {
    ...
    it('matches scale in thirds', () => {
      const result = generateFromDemand({ description: 'C major scale in thirds' });
      expect(result).not.toBeNull();
      expect(result.title).toContain('thirds');
    });
    ...
    it('matches interval demands', () => {
      const result = generateFromDemand({ description: 'octave jump exercises' });
      expect(result).not.toBeNull();
      expect(result.category_hint).toBe('Intervals');
    });
    ...
    it('matches tonguing demands', () => {
      const result = generateFromDemand({ description: 'double tonguing drill' });
      expect(result).not.toBeNull();
      expect(result.category_hint).toBe('Articulation');
    });

    it('returns null for unrecognized demands', () => {
      const result = generateFromDemand({ description: 'improve vibrato width' });
      expect(result).toBeNull();
    });

    it('extracts key from description', () => {
      const result = generateFromDemand({ description: 'Bb minor scale practice' });
      expect(result).not.toBeNull();
      expect(result.key).toBe('Bb');
    });

    it('uses provided key over extracted key', () => {
      const result = generateFromDemand({ description: 'scale practice', key: 'F#' });
    ...
    it('extracts harmonic minor from description', () => {
      const result = generateFromDemand({ description: 'harmonic minor scale' });
      expect(result).not.toBeNull();
