...
import { requireAuth, createToken, verifyToken, optionalAuth } from '../../middleware/auth.js';
...
describe('auth middleware', () => {
  const testUser = { id: 'u1', email: 'test@example.com', display_name: 'Test' };
  ...
  describe('requireAuth', () => {
    it('passes with valid JWT in cookie', () => {
      const token = createToken(testUser);
      const { req, res, next } = makeMockReqRes({ cookies: { pf_token: token } });
    ...
    it('prefers Authorization header over cookie', () => {
      const headerUser = { id: 'h1', email: 'header@test.com', display_name: 'Header' };
      const cookieUser = { id: 'c1', email: 'cookie@test.com', display_name: 'Cookie' };
      const { req, res, next } = makeMockReqRes({
    ...
    it('rejects malformed JWT', () => {
      const { req, res, next } = makeMockReqRes({ cookies: { pf_token: 'not.a.jwt' } });
      requireAuth(req, res, next);
      expect(res.status).toHaveBeenCalledWith(401);
      expect(next).not.toHaveBeenCalled();
    });
  ...
  describe('optionalAuth', () => {
    ...
    it('falls back to dev user when token is invalid and AUTH_REQUIRED is not true', () => {
      delete process.env.AUTH_REQUIRED;
      const { req, res, next } = makeMockReqRes({ cookies: { pf_token: 'bad' } });
      optionalAuth(req, res, next);
      expect(next).toHaveBeenCalled();
      expect(req.user.id).toBe('dev-user');
    });
