...
const CHALLENGE = { id: 'ch-1', creator_id: 'test-user-id', type: 'duel', status: 'active' };
...
describe('Challenges Routes', () => {
  describe('POST /api/challenges', () => {
    it('should create a challenge', async () => {
      ...
      mockQuery('SELECT * FROM challenges WHERE id', { ...CHALLENGE, id: 'test-uuid-1' });
      mockQuery('SELECT cp.*, u.display_name', [{ user_id: 'test-user-id', display_name: 'Test', status: 'accepted' }]);
    ...
    it('should return 401 without token', async () => {
      const res = await request(app).post('/api/challenges').send({ type: 'duel' });
  ...
  describe('PUT /api/challenges/:id/cancel', () => {
    it('should cancel a challenge (creator only)', async () => {
      ...
      mockQuery('SELECT * FROM challenges WHERE id', { ...CHALLENGE, status: 'cancelled' });
    ...
    it('should return 403 for non-creator', async () => {
      mockQuery('SELECT * FROM challenges WHERE id', { ...CHALLENGE, creator_id: 'other-user' });
  ...
  describe('PUT /api/challenges/:id/accept', () => {
    it('should accept an invitation', async () => {
      mockQuery("SELECT * FROM challenge_participants WHERE challenge_id", { id: 'cp1', status: 'invited' });
      mockQuery('UPDATE challenge_participants', []);
      mockQuery("SELECT id FROM challenge_participants WHERE challenge_id", null);
      mockQuery('UPDATE challenges', []);
      mockQuery('SELECT * FROM challenges WHERE id', { ...CHALLENGE, status: 'active' });
  ...
  describe('PUT /api/challenges/:id/submit', () => {
    it('should submit to a challenge', async () => {
      mockQuery("SELECT * FROM challenge_participants WHERE challenge_id", { id: 'cp1', status: 'accepted' });
      mockQuery('UPDATE challenge_participants SET', []);
      mockQuery("SELECT id FROM challenge_participants WHERE challenge_id", null);
      // All submitted - ranking
      mockQuery('SELECT cp.*, u.display_name FROM challenge_participants', [{ id: 'cp1', user_id: 'test-user-id', display_name: 'Test', score: 90 }]);
      mockQuery('UPDATE challenge_participants SET rank', []);
      mockQuery('UPDATE challenges SET status', []);
      mockQuery('INSERT INTO feed_events', []);
      mockQuery('SELECT * FROM challenges WHERE id', { ...CHALLENGE, status: 'completed' });
      mockQuery('SELECT cp.*, u.display_name', []);
  ...
  describe('POST /api/challenges/weekly/generate', () => {
    it('should generate a weekly challenge', async () => {
      mockQuery('SELECT * FROM excerpts ORDER BY RANDOM', { id: 'exc-1', title: 'Test', composer: 'Bach' });
      mockQuery('INSERT INTO challenges', []);
      mockQuery('INSERT INTO challenge_participants', []);
      mockQuery('SELECT * FROM challenges WHERE id', { id: 'test-uuid-1', type: 'weekly', status: 'active' });
      mockQuery('SELECT cp.*, u.display_name', []);
