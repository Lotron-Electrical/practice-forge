...
beforeAll(async () => {
  app = await createApp('../routes/communityExcerpts.js', '/api/community-excerpts');
  token = generateToken();
});
...
describe('Community Excerpts Routes', () => {
  describe('GET /api/community-excerpts', () => {
    it('should return excerpts with community data', async () => {
      mockQuery('SELECT e.*', [{ id: 'e1', title: 'Test', avg_difficulty: 5, rating_count: 3, note_count: 1 }]);
  ...
  describe('GET /api/community-excerpts/:id/community', () => {
    it('should return community data for an excerpt', async () => {
      mockQuery('SELECT COALESCE(AVG', { avg_difficulty: 5, rating_count: '3' });
      mockQuery('SELECT ecn.*, u.display_name', []);
      mockQuery('SELECT difficulty_rating FROM excerpt_community_ratings', null);
  ...
  describe('POST /api/community-excerpts/:id/notes', () => {
    it('should add a note', async () => {
      ...
      mockQuery('SELECT * FROM excerpt_community_notes WHERE id', { id: 'test-uuid-1', note: 'Great excerpt' });
  ...
  describe('DELETE /api/community-excerpts/notes/:noteId', () => {
    it('should delete own note', async () => {
      mockQuery('SELECT * FROM excerpt_community_notes WHERE id', { id: 'n1', user_id: 'test-user-id' });
    ...
    it('should return 403 for another user note', async () => {
      mockQuery('SELECT * FROM excerpt_community_notes WHERE id', { id: 'n1', user_id: 'other-user' });
  ...
  describe('POST /api/community-excerpts/notes/:noteId/upvote', () => {
    it('should upvote a note', async () => {
      mockQuery('SELECT id FROM excerpt_community_notes WHERE id', { id: 'n1' });
      mockQuery('UPDATE excerpt_community_notes SET upvotes', []);
      mockQuery('SELECT * FROM excerpt_community_notes WHERE id', { id: 'n1', upvotes: 2 });
