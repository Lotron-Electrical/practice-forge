...
const SESSION = { id: 'sess-1', date: '2025-01-01', planned_duration_min: 60, status: 'planned' };

describe('Sessions Routes', () => {
  describe('GET /api/sessions', () => {
    it('should return sessions with blocks', async () => {
      mockQuery('SELECT * FROM practice_sessions ORDER BY', [SESSION]);
      mockQuery('SELECT * FROM session_blocks WHERE session_id', [{ id: 'b1', session_id: 'sess-1', category: 'warmup' }]);
  ...
  describe('GET /api/sessions/rotation/today', () => {
    it('should return today rotation', async () => {
      mockQuery('SELECT rl.*, e.title', [{ id: 'r1', excerpt_id: 'exc-1', title: 'Test' }]);

      const res = await request(app).get('/api/sessions/rotation/today').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(1);
    });
  });
  ...
  describe('POST /api/sessions/rotation/:rotationId/practiced', () => {
    it('should mark rotation as practiced', async () => {
      mockQuery('SELECT * FROM excerpt_rotation_log WHERE id', { id: 'r1', excerpt_id: 'exc-1' });
  ...
  describe('POST /api/sessions/generate', () => {
    it('should generate a practice session', async () => {
      mockQuery("SELECT value FROM settings WHERE key = 'timeAllocation'", null);
      // Fundamentals query
      ...
      // Return session
      mockQuery('SELECT * FROM practice_sessions WHERE id', { ...SESSION, id: 'test-uuid-1' });
      mockQuery('SELECT * FROM session_blocks WHERE session_id', []);
  ...
  describe('PUT /api/sessions/:id/start', () => {
    it('should start a session', async () => {
      ...
      mockQuery('SELECT * FROM practice_sessions WHERE id', { ...SESSION, status: 'in_progress' });
      mockQuery('SELECT * FROM session_blocks WHERE session_id', []);
  ...
  describe('PUT /api/sessions/:id/complete', () => {
    it('should complete a session', async () => {
      mockQuery('SELECT * FROM practice_sessions WHERE id', SESSION);
      mockQuery('SELECT * FROM session_blocks WHERE session_id', [{ planned_duration_min: 10 }]);
      mockQuery('UPDATE practice_sessions', []);
      mockQuery('SELECT * FROM practice_sessions WHERE id', { ...SESSION, status: 'completed' });
      mockQuery('SELECT * FROM session_blocks WHERE session_id', []);
  ...
  describe('GET /api/sessions/stats', () => {
    it('should return practice stats', async () => {
      mockQuery("SELECT * FROM practice_sessions WHERE date >= ", [{ actual_duration_min: 60 }]);
      mockQuery('SELECT DISTINCT date FROM practice_sessions', [{ date: new Date().toISOString().slice(0, 10) }]);

      const res = await request(app).get('/api/sessions/stats').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
