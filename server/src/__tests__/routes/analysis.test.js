...
const FILE = {
  id: "file-1",
  file_type: "sheet_music_digital",
  file_path: "scores/test.xml",
};
const OMR = {
  id: "omr-1",
  file_id: "file-1",
  musicxml_path: "scores/test.xml",
  confidence: 0.95,
};
const ANALYSIS = {
  id: "an-1",
...
describe("Analysis Routes", () => {
  ...
  describe("POST /api/analysis/trigger-claude/:analysisId", () => {
    it("should return cost estimate when not confirmed", async () => {
      mockQuery("SELECT * FROM analysis_results WHERE id", {
        ...ANALYSIS,
        file_id: "file-1",
      });
      mockQuery("SELECT * FROM uploaded_files WHERE id", FILE);
  ...
  describe("GET /api/analysis/results/:fileId", () => {
    it("should return analysis result", async () => {
      ...
      const res = await request(app)
        .get("/api/analysis/results/file-1")
        .set("Authorization", `Bearer ${token}`);
      expect(res.status).toBe(200);
    });
  ...
  describe("POST /api/analysis/demands/:demandId/import", () => {
    it("should import a demand", async () => {
      mockQuery("SELECT * FROM analysis_demands WHERE id", {
        id: "d1",
        description: "Trill",
        category_id: null,
        difficulty: 5,
  ...
  describe("GET /api/analysis/status/:fileId", () => {
    it("should return combined status", async () => {
      ...
      const res = await request(app)
        .get("/api/analysis/status/file-1")
        .set("Authorization", `Bearer ${token}`);
      expect(res.status).toBe(200);
