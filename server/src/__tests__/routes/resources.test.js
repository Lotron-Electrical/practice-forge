...
const RESOURCE = { id: 'res-1', linked_type: 'piece', linked_id: 'p1', resource_type: 'score', title: 'IMSLP Score', url: 'https://imslp.org/test', source: 'imslp' };
...
describe('Resources Routes', () => {
  describe('GET /api/resources', () => {
    ...
    it('should return 401 without token', async () => {
      const res = await request(app).get('/api/resources?linked_type=piece&linked_id=p1');
  ...
  describe('POST /api/resources', () => {
    it('should create a resource', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', `Bearer ${token}`)
        .send({
          linked_type: 'piece', linked_id: 'p1', resource_type: 'score',
          title: 'IMSLP Score', url: 'https://imslp.org/test', source: 'imslp',
        });
  ...
  describe('DELETE /api/resources/:id', () => {
    it('should delete a resource', async () => {
      ...
      const res = await request(app).delete('/api/resources/res-1').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
      expect(res.body.deleted).toBe(true);
    });

    it('should return 404 for missing resource', async () => {
      const res = await request(app).delete('/api/resources/nope').set('Authorization', `Bearer ${token}`);
  ...
  describe('GET /api/resources/search/imslp', () => {
    it('should return 400 without q param', async () => {
      const res = await request(app).get('/api/resources/search/imslp').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('GET /api/resources/search/wikipedia', () => {
    it('should return 400 without q param', async () => {
      const res = await request(app).get('/api/resources/search/wikipedia').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('GET /api/resources/search/youtube', () => {
    it('should return 400 without q param', async () => {
      const res = await request(app).get('/api/resources/search/youtube').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(400);
    });
