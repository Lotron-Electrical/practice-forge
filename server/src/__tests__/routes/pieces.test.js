...
const PIECE = { id: 'piece-1', title: 'Sonata', composer: 'Mozart', status: 'not_started', priority: 'medium', general_notes: '' };
...
describe('Pieces Routes', () => {
  ...
  describe('GET /api/pieces', () => {
    it('should return all pieces with sections and demands', async () => {
      ...
      mockQuery('SELECT * FROM sections WHERE piece_id', [{ id: 's1', piece_id: 'piece-1', name: 'Mvt 1' }]);
      mockQuery('SELECT * FROM technical_demands WHERE piece_id', []);

      const res = await request(app).get('/api/pieces').set('Authorization', `Bearer ${token}`);
  ...
  describe('GET /api/pieces/:id', () => {
    it('should return a piece with full details', async () => {
      ...
      const res = await request(app).get('/api/pieces/piece-1').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
      expect(res.body.title).toBe('Sonata');
    });

    it('should return 404 for non-existent piece', async () => {
      const res = await request(app).get('/api/pieces/nope').set('Authorization', `Bearer ${token}`);
  ...
  describe('POST /api/pieces', () => {
    it('should create a piece', async () => {
      ...
      mockQuery('SELECT * FROM pieces WHERE id', { ...PIECE, id: 'test-uuid-1' });
  ...
  describe('PUT /api/pieces/:id', () => {
    it('should update a piece', async () => {
      ...
      mockQuery('SELECT * FROM pieces WHERE id', { ...PIECE, title: 'Updated' });
  ...
  describe('DELETE /api/pieces/:id', () => {
    it('should delete a piece', async () => {
      ...
      const res = await request(app).delete('/api/pieces/piece-1').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(200);
      expect(res.body.deleted).toBe(true);
    });

    it('should return 404 for non-existent piece', async () => {
      const res = await request(app).delete('/api/pieces/nope').set('Authorization', `Bearer ${token}`);
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('POST /api/pieces/:pieceId/sections', () => {
    it('should create a section', async () => {
      ...
      mockQuery('SELECT * FROM sections WHERE id', { id: 'test-uuid-1', name: 'Mvt 1' });
  ...
  describe('POST /api/pieces/:pieceId/demands', () => {
    it('should create a demand', async () => {
      ...
      mockQuery('SELECT * FROM technical_demands WHERE id', { id: 'test-uuid-1', description: 'Trills' });
