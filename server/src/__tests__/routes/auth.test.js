...
import { createApp, generateToken, mockQuery, mockQueryOnce } from '../setup.js';

let app;

beforeAll(async () => {
  app = await createApp('../routes/auth.js', '/api/auth', { requireAuth: false });
});
...
describe('Auth Routes', () => {
  ...
  describe('POST /api/auth/register', () => {
    it('should register a new user', async () => {
      ...
      mockQuery('SELECT id, email, display_name', {
        id: 'test-uuid-1', email: 'new@example.com', display_name: 'new',
        instrument: 'Flute', level: 'student', created_at: new Date().toISOString(),
      });
  ...
  describe('POST /api/auth/login', () => {
    it('should login with valid credentials', async () => {
      mockQuery('SELECT * FROM users WHERE email', {
        id: 'user-1', email: 'test@example.com', password_hash: 'hashed_password123',
        display_name: 'Test', instrument: 'Flute', level: 'student',
      });
    ...
    it('should return 401 for wrong password', async () => {
      mockQuery('SELECT * FROM users WHERE email', {
        id: 'user-1', email: 'test@example.com', password_hash: 'hashed_correctpassword',
      });
  ...
  describe('GET /api/auth/me', () => {
    ...
    it('should return user profile with valid token', async () => {
      ...
      mockQuery('SELECT id, email, display_name', {
        ...
        instrument: 'Flute', level: 'student',
      });
  ...
  describe('PUT /api/auth/me', () => {
    it('should update profile with valid token', async () => {
      ...
      mockQuery('SELECT * FROM users WHERE id', {
        id: 'user-1', display_name: 'Old', instrument: 'Flute', level: 'student',
      });
      mockQuery('UPDATE users', []);
      mockQuery('SELECT id, email, display_name', {
        id: 'user-1', display_name: 'New Name', instrument: 'Clarinet', level: 'student',
      });
  ...
  describe('PUT /api/auth/me/password', () => {
    ...
    it('should change password with correct current password', async () => {
      const token = generateToken({ id: 'user-1' });
      mockQuery('SELECT * FROM users WHERE id', {
        id: 'user-1', password_hash: 'hashed_oldpassword',
      });
    ...
    it('should return 401 for wrong current password', async () => {
      const token = generateToken({ id: 'user-1' });
      mockQuery('SELECT * FROM users WHERE id', {
        id: 'user-1', password_hash: 'hashed_correctpassword',
      });
