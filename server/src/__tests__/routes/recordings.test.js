...
describe("Recordings Routes", () => {
  ...
  describe("DELETE /api/recordings/:id", () => {
    it("should delete recording and linked file", async () => {
      mockQuery("SELECT * FROM audio_recordings WHERE id", {
        id: "rec-1",
        file_id: "f1",
      });
      mockQuery("SELECT * FROM uploaded_files WHERE id", {
        id: "f1",
        file_path: "recordings/test.mp3",
      });
  ...
  describe("POST /api/recordings/:id/analysis", () => {
    it("should save analysis results", async () => {
      ...
      mockQuery("SELECT * FROM audio_analyses WHERE id", {
        id: "test-uuid-1",
        pitch_accuracy_pct: 92.5,
      });
  ...
  describe("GET /api/recordings/:id/analysis", () => {
    it("should return analysis", async () => {
      mockQuery("SELECT * FROM audio_analyses WHERE recording_id", {
        id: "a1",
        pitch_accuracy_pct: 92,
      });
      const res = await request(app)
        .get("/api/recordings/rec-1/analysis")
        .set("Authorization", `Bearer ${token}`);
      expect(res.status).toBe(200);
    });
    ...
    it("should return 404 when no analysis", async () => {
      const res = await request(app)
        .get("/api/recordings/nope/analysis")
        .set("Authorization", `Bearer ${token}`);
      expect(res.status).toBe(404);
    });
  });
